...
🔴 **spec の自由文で gh コマンドに触れるときは、コマンドの直後に理由を置く** (#729)。
`summary` / `extraVerification` / `extraProhibitions` に 403 になる実行形を書くと **exit 3** で
止まる（生成器が「使わないこと」と書いている隣で壊れたコマンドを配らせないため）。
`` `gh pr create` は使わないこと `` / `` `gh pr view --head` が 403 でした `` は通るが、
理由が離れていると落ちる（`403 は解消済みなので \`gh pr create\` で作る` 等）。
緩和語彙（`可能なら` 等）は ⚠ が出るだけで止まらない。
