 ...
 *
 * 管理操作（ログイン・端末作成・トークン発行）は使い捨ての admin コンテキストで行い、`page` には
 * kiosk セッション cookie だけを残す（実機の受付端末に管理セッションが乗らないのと同じ）。fullyParallel
 * 下の干渉を避けるため、テストごとに一意な端末を新規作成してエンロールする。
 */
export async function establishKioskSession(page: Page, browser: Browser): Promise<void> {
  // browser.newContext() は config の use.baseURL を継承しないため、明示的に渡す
  // （playwright.config.ts と同じ解決）。相対 URL の request を成立させるため。
  const baseURL =
    process.env.PLAYWRIGHT_BASE_URL?.replace(/\/$/, '') ??
    `http://127.0.0.1:${process.env.PORT ?? 3000}`;
  const admin = await browser.newContext({ baseURL });
  try {
    const login = await admin.request.post('/api/admin/login', {
      data: { password: 'open-reception' },
    });
    expect(login.ok()).toBeTruthy();

    const created = await admin.request.post('/api/admin/devices', {
      data: {
        tenantId: 'internal',
        siteId: 'default-site',
    ...
    const issued = await admin.request.post(`/api/admin/devices/${deviceId}/reissue-token`, {
