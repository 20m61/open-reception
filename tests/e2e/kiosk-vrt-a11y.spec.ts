...
test.describe('デモプレビュー経由の画面（営業時間外・サイネージ・iPad landscape）', () => {
  ...
  test('サイネージ待機画面の a11y（カルーセル/時計のため VRT 対象外）', async ({ page }) => {
    ...
    // 以前は `nested-interactive`（check: `no-focusable-content`）の既知 serious 違反を 1 ルール
    // 除外していた（`signage-display` が role="button" tabindex="0" を持ちつつ内側に focusable な
    // `signage-start` ボタンを内包）。SignageDisplay 側で外側の button role/tabindex を撤去し
    // 非対話コンテナへ変更したため除外は不要になり、serious 全ルールを検査する。
    const violations = await blockingViolations(page);
    expect(violations, summarize(violations)).toEqual([]);
  });
});
