...
test.describe('来訪者導線: 応答が返らなくても退館の手段を失わない (#1029)', () => {
  ...
  /**
   * 🔴 **この spec は 2026-09-09 に主張を反転した（#1042）。**
   *
   * 元は「接続そのものが失敗したときは、再試行を促す（受付へ回さない）」であり、
   * `route.abort('failed')` に対して `通信エラー` を期待していた。根拠は実装コメントの
   * 「接続そのものが失敗した（＝サーバに**届いていない**）」だったが、**これは証明できない**。
   *
   * `fetch` が `TypeError` で reject するのは
   * ・要求を送る**前**の失敗（DNS 解決失敗・接続拒否・TLS ハンドシェイク失敗）
   * ・要求を送った**後**の失敗（応答待ちの接続リセット、経路の切断）
   * の**両方**で、ブラウザは両者を区別する情報を出さない。`route.abort('failed')` も
   * 同じく区別しない。**つまり元の spec は、観測できない前提を測らずに固定していた**
   * （`.claude/rules/opus5-autonomous-loop.md`「散文で書いた保証は、測った範囲を超えない」）。
   *
   * 後者が起きると、**サーバは退館を受理して監査に残しているのに**画面が
   * 「もう一度お試しください」と促し、来訪者は再送して `already_checked_out` を踏む。
   * #968 が `read-response.ts` に明文化した「**書き込みの中断は『失敗した』と言い切らない**」
   * に反する。よって**確定的な応答が無い限り**「分からない」へ倒す（人間承認済み: #1042）。
   *
   * 受け入れたコスト: Wi-Fi 断で本当に届かなかった場合も、来訪者を不要に受付へ歩かせる。
   * **読み取り（自己特定 `resolve`）はこの反転の対象外**で `通信エラー` のままが正しい ――
   * あちらは何も確定しないので再試行すれば済む。その下界は下の
   * **`退館: 自己特定の接続そのものが失敗したときは、通信の問題として伝える`** が持つ
   * （行番号で指さない ―― 編集のたびに腐る。独立レビュー 1 周目 MINOR-1）。
   */
  test('退館確定: 接続が失敗しても、届いていないと決めつけずに受付へ繋ぐ (#1042)', async ({ page }) => {
    const PRESENT = JSON.stringify({
      stays: [
        {
          stayId: 'off1',
    ...
    await page.route('**/api/kiosk/checkout', (route) => {
      ...
      posts += 1;
      /*
        🔴 **注入そのものは「送信前」である**（独立レビュー 1 周目 MINOR-2 の訂正）。
        `route.abort('failed')` は Playwright の傍受点で落とすので、要求はサーバへ**出ない**。
        区別できないのは事象ではなく、**アプリから見える signal のほう**である ――
        ページに届くのは送信後のリセットと同一の `TypeError` で、`fetch` はこれを
        意図的に不透明にしている（cross-origin の情報漏洩を防ぐための仕様）。
        よってこの spec が固定するのは「区別できない signal を受けたときの振る舞い」である。
        **サーバが実際に受理した場合**は下の `route.fetch()` を使う spec が測る。
      */
      return route.abort('failed');
    });
    ...
    const error = page.getByTestId('checkout-error');
    await expect(error).toBeVisible();
    // **これが本題。** 届いたかどうか分からないので、成功を否定せず受付へ繋ぐ。
    await expect(error).toContainText('確認できませんでした');
    await expect(error).toContainText('受付');
    /*
      🔴 **下界。** 「もう一度お試しください」へ戻す変異をここで落とす。
      サーバが受理しているかもしれない状態で再送を促すと、`already_checked_out` を踏ませる。
    */
    await expect(error).not.toContainText('もう一度お試しください');
  });
