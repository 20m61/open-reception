...
describe('ゲートの一時領域レポート (#1136)', () => {
   ...
   *
   * テストファイルごとの一時領域は `<tmp>/or-vitest/XXXXXX` に入るので、
   * **直下の総数からは 1 件にしか見えない** —— kill された run の残骸が、
   * いちばん見せたい状況で不可視になる。ここだけ潜って数えることを縛る。
   */
  it('🔴 隔離 root の中身を 1 段潜って数える（直下の 1 件で隠れない）', () => {
    // 🔴 **名前は隔離の実体から採る（レビュー 3 周目 MINOR 2）。** 直書きだと、
    //    `RUN_ROOT` を改名したときに**ゲートだけが古い名前を数え続け**、
    //    `unit 隔離 root 0 件` を永久に表示する（この PR で実際に 1 度、
    //    別のハーネスが同じ形で空虚に通った）。`tmpdir()` は
    //    `<tmp>/<隔離 root 名>/XXXXXX` なので、その親の basename が名前である。
    const isolationRoot = basename(dirname(tmpdir()));
    const out = runReport([`${isolationRoot}/a`, `${isolationRoot}/b`, 'other']);
    expect(out).toContain('unit 隔離 root 2 件');
    // 下界: 直下のほうは 2 件（隔離 root と `other`）として数えていること。
