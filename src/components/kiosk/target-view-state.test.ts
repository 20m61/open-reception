...
function panel(over: Partial<Parameters<typeof targetPanelFor>[0]> = {}): TargetPanel {
  ...
  // 「選べる件数 > カードの件数」は起こり得ない。黙って丸めると、書いた値と評価される
  // 値が食い違う別種の嘘になる。到達不能な入力はテスト側で落とす。
  expect(input.selectableStaffCount).toBeLessThanOrEqual(input.staffResultCount);
  return targetPanelFor(input);
}
...
describe('相手選択の表示モード (#776)', () => {
  ...
  it('担当者タブでヒットが有れば担当者グリッドだけを出す', () => {
    expect(panel({ tab: 'staff', staffResultCount: 1, selectableStaffCount: 1 })).toEqual({
      kind: 'staff-results',
    });
  });
  ...
  it('部署タブでは担当者が 0 件でも recovery ではなく部署グリッドを出す', () => {
    ...
    expect(panel({ tab: 'department', staffResultCount: 0, selectableStaffCount: 0, searching: true })).toEqual({
      kind: 'departments',
    });
  });
});
...
describe('0 件からの回復 (#776 / #322 AC3)', () => {
  it('検索して 0 件なら recovery パネル 1 つに集約する（警告と案内を重ねない）', () => {
    expect(panel({ staffResultCount: 0, selectableStaffCount: 0, searching: true })).toEqual({
      kind: 'recovery',
  ...
  it('検索していないのに 0 件（名簿が空）なら「別の名前で」ではなく不在案内を出す', () => {
    expect(panel({ staffResultCount: 0, selectableStaffCount: 0, searching: false })).toEqual({
      kind: 'recovery',
  ...
  it('次の一手は他方のタブが先、チャットが後（主導線と競合させない）', () => {
    const result = panel({ staffResultCount: 0, selectableStaffCount: 0, searching: true });
    expect(result.kind === 'recovery' && result.actions[0]).toBe('department');
  });

  it('チャットを出せなければチャットの次の一手を出さない', () => {
    expect(panel({ staffResultCount: 0, selectableStaffCount: 0, searching: true, chatAvailable: false })).toEqual({
      kind: 'recovery',
      messageKey: 'reception.searchNoResultsGuidance',
      actions: ['department'],
    });
  ...
  it('部署が 1 つも無ければ「部署から選ぶ」を出さない（空パネルへ送らない）', () => {
    ...
    expect(panel({ staffResultCount: 0, selectableStaffCount: 0, searching: true, departmentCount: 0 })).toEqual({
      kind: 'recovery',
      messageKey: 'reception.searchNoResultsGuidance',
      actions: ['chat'],
    });
  ...
  it('部署を出せないなら「部署または代表窓口をお選びください」と言わない', () => {
    ...
    // 行き止まりへ言葉で案内することになる。
    expect(panel({ staffResultCount: 0, selectableStaffCount: 0, searching: false, departmentCount: 0 })).toEqual({
      kind: 'recovery',
  ...
  it('担当者タブが返すのは results か recovery のどちらか一方だけ', () => {
    for (const count of [0, 1, 2, 10]) {
      for (const searching of [true, false]) {
        const result = panel({
          tab: 'staff',
          staffResultCount: count,
          selectableStaffCount: count,
          searching,
        });
