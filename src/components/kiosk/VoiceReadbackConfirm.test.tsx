...
describe('VoiceReadbackConfirm (#361 音声復唱 UI / #364 字幕・インジケータ)', () => {
  ...
  it('部署の復唱は担当者用テンプレートを使わず専用文言を出す（issue #361 申し送り: 「営業部様ですね？」の不自然さを解消）', () => {
    const html = render({
      mode: 'readback',
      readbackName: '営業部',
      readbackReason: 'low_stt_confidence',
      readbackKind: 'department',
    });
    expect(html).toContain('営業部でよろしいですか？');
    expect(html).not.toContain('営業部様ですね？');
    expect(html).not.toContain('様');
  });

  it('部署の復唱は 4 言語で描画できる（i18n）', () => {
    const dept = (locale: 'en' | 'ko' | 'zh') =>
      render(
        { mode: 'readback', readbackName: '営業部', readbackReason: 'low_stt_confidence', readbackKind: 'department' },
        locale,
      );
    // NOTE: readbackName（表示名）は組織辞書由来の値をそのまま渡す（翻訳しない）。
    // 言語別に変わるのはテンプレート文言のみ。
    expect(dept('en')).toContain('Would you like to be connected to 営業部?');
    expect(dept('ko')).toContain('営業部 부서가 맞으신가요?');
    expect(dept('zh')).toContain('您要找的是営業部吗？');
  });
