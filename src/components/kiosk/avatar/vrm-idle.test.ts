...
describe('vrm-idle (#31 / #1085)', () => {
  ...
  it('naturalMicroMotion は caller-owned target を再利用できる', () => {
    const target = { headX: 99, headY: 99, neckX: 99, neckY: 99, chestY: 99 };
    const sampled = naturalMicroMotion(12.34, 42, target);
    expect(sampled).toBe(target);
    expect(sampled).toEqual(naturalMicroMotion(12.34, 42));
  });
