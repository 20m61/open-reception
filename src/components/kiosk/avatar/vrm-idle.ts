...
export type MutableNaturalMicroMotion = {
  headX: number;
  headY: number;
  neckX: number;
  neckY: number;
...
export type NaturalMicroMotion = Readonly<MutableNaturalMicroMotion>;
...
export function naturalMicroMotion(
  elapsedSec: number,
  seed = DEFAULT_NATURAL_MOTION_SEED,
  target?: MutableNaturalMicroMotion,
): NaturalMicroMotion {
  const result =
    target ?? ({ headX: 0, headY: 0, neckX: 0, neckY: 0, chestY: 0 } satisfies MutableNaturalMicroMotion);
  result.headX = smoothValueNoise(elapsedSec * 0.22, seed + 71) * 0.006;
  result.headY = smoothValueNoise(elapsedSec * 0.19, seed + 89) * 0.008;
  result.neckX = smoothValueNoise(elapsedSec * 0.16, seed + 107) * 0.003;
  result.neckY = smoothValueNoise(elapsedSec * 0.14, seed + 131) * 0.004;
  result.chestY = smoothValueNoise(elapsedSec * 0.09, seed + 149) * 0.006;
  return result;
}
