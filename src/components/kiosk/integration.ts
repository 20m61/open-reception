...
export type KioskGate = 'revoked' | 'authorize' | 'unenrolled' | 'ready';
 ...
 *   - `authorized===null`（heartbeat 取得前）は楽観的に `'ready'`。確立済み端末の初期フラッシュを
 *     避けるためで、未保持端末も idle 待機画面に PII は無く、次の heartbeat でゲートが閉じる。
 */
