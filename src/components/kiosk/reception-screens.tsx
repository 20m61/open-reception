 ...
 *
 * 1 画面 1 主目的: 「何のご用件か」を大きなカードで選ぶ。音声・チャットなしでもタッチだけで
 * 進める。
 *
 * 入口の集合・並び順・用件の先取りは**契約が決める** (#422 inc5-b 増分 3b)。
 *  - `turnAnswersFor('idle', locale)` … 受付を開始する入口（用件の先取りは `presetPurpose`）
 *  - `turnHandoffsFor('idle', locale)` … QR 受付への引き渡し（状態機械を進めず別シェルへ）
 * 画面が足すのはアイコンと説明文だけ。
 *
 * 後方互換: 既存 E2E/テストが参照する `start-reception`（担当者を呼ぶ）と
 * `start-checkin`（QR で受付）の testid は、登録簿（`conversation-turn.ts`）が維持する。
 */
...
export function TargetView({
  ...
  const [tab, setTab] = useState<TargetTab>(DEFAULT_TARGET_TAB);
  ...
  return (
    <>
      ...
      <div className="screen__body">
        ...
        */}
        <div className="target-tabs" role="tablist" aria-label={tr('reception.targetTabsLabel')}>
          {TARGET_TABS.map((id) => (
            <button
              key={id}
              type="button"
        ...
        {panel.kind === 'departments' ? (
          <div
            role="tabpanel"
            id="target-panel-department"
            aria-labelledby="target-tab-department"
            data-testid="target-panel-department"
          >
            <div className="card-grid">
              {departments.map((d) => (
                <button
          ...
          <div
            role="tabpanel"
            id="target-panel-staff"
            aria-labelledby="target-tab-staff"
            data-testid="target-panel-staff"
          >
            <div className="field">
            ...
            {panel.kind === 'staff-results' ? (
              ...
              // 次の一手（部署 → チャット）を優先順に並べる (#322 AC3 / #776)。
                  ...
                  {panel.actions.map((action) =>
                    action === 'department' ? (
                      <button
                        ...
                        // 1 操作で部署タブへ切り替える（スクロール誘導では「探しに行く」操作が要る）。
