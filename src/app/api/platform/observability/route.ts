...
import { deriveConnectivity, type DeviceConnectivity } from '@/lib/tenant/device-service';
 ...
 *     online/offline/maintenance/disabled）。件数のみ・PII なし。
 *
 * 未接続（次増分で指標ソースへ接続）:
...
export async function GET(): Promise<NextResponse> {
  ...
  // 当月分のみ必要なため境界クエリで取得（全件走査を避ける, #254）。端末は全テナント横断の実死活集計。
  // 監視画面なので取得失敗は握り潰さず伝播させる（偽の健全表示を出さない。UI は !res.ok で明示）。
  const [integrations, auditLogs, receptionLogs, allDevices] = await Promise.all([
  ...
  // 端末の実死活は heartbeat(lastSeenAt) 由来（deriveConnectivity。5 分窓）。有効フラグではなく実活動。
  const byConn: Record<DeviceConnectivity, number> = { online: 0, offline: 0, maintenance: 0, disabled: 0 };
  for (const d of allDevices) byConn[deriveConnectivity(d, now)] += 1;
  ...
  return NextResponse.json({
    ...
    // 端末の実死活（heartbeat=lastSeenAt 由来）。online=直近 5 分に heartbeat 受信。
    devices: { total: allDevices.length, ...byConn },
    metrics: {
