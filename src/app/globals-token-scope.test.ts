...
describe('デザイントークンのスコープ (#501)', () => {
  it('既定(:root)は受付端末向けのまま（kiosk の実寸を動かさない）', () => {
    ...
    expect(declaredValue(':root', '--font-body')).toContain('1.25rem');
    expect(declaredValue(':root', '--font-lg')).toContain('1.75rem');
    expect(declaredValue(':root', '--font-xl')).toContain('2.75rem');
  });
  ...
  it('管理画面スコープが全サイズトークンを下げる（継承を断つ）', () => {
    ...
    expect(declaredValue(ADMIN_SCOPE, '--font-body')).toContain('1rem');
  });
