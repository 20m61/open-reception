/**
 * /notify リクエストの入力検証 (DESIGN #34 §5-1 / #275 で domain へ集約)。
 *
 * すべて純関数（フレームワーク/AWS SDK import 禁止）。送信側(app)・受信側(worker Lambda)の
 * 双方がこの単一実装を参照する（schema-consistency.test.ts で参照同一性を固定）。
 * 不正入力は worker handler 側で 400 を返す。
 */
import type { NotificationRequest, NotificationKind, NotificationTarget } from './notify';
