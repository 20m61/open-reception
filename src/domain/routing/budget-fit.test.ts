...
describe('routingFitsClientWait (#743)', () => {
  ...
  it('🔴 hop 上限まで使った最悪ケースが端末上限に収まる呼出時間の上限を示す', () => {
    ...
    // 余裕 30s が 1 手あたりの予算を食うので、現状では**負**になる
    // （5 分 / 10 手 = 30s/手 で、余裕だけで使い切る）。この事実を明示的に固定する。
    expect(maxTimeoutSeconds).toBeLessThanOrEqual(0);
    // よって hop 上限まで使い切る構成は原理的に端末上限へ収まらない。
    expect(routingFitsClientWait(Array.from({ length: DEFAULT_MAX_HOPS }, () => 1))).toBe(false);
  });
});
