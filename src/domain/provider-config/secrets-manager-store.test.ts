...
describe('削除猶予中の整合（敵対的レビュー I2）', () => {
  it('get は削除猶予中(InvalidRequestException)を null に正規化し presence=missing と整合する', async () => {
    const err = new Error('marked for deletion');
    ...
    const store = new SecretsManagerTenantSecretStore(backend, 'open-reception/test');
    await expect(store.getSecret('tenants/t1/vonage')).resolves.toBeNull();
    await expect(store.hasSecret('tenants/t1/vonage')).resolves.toBe(false);
  });
});
