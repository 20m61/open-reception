...
describe('DEMO_SCENARIOS (issue #363 初期 9 シナリオ + #364 音声成功系)', () => {
  it('11 シナリオを seed する（初期 9 + 音声成功系 2: 担当者/部門）', () => {
  ...
  it('音声成功系は担当者解決（voice-staff-visit）と部門解決（voice-department-visit）の両方を seed する (#364)', () => {
    const staffScenario = getDemoScenario('voice-staff-visit');
    const deptScenario = getDemoScenario('voice-department-visit');
    expect(staffScenario).toBeDefined();
    expect(deptScenario).toBeDefined();
    expect(staffScenario?.simulatedResults.stt).toBe('success');
    expect(deptScenario?.simulatedResults.stt).toBe('success');
    expect(deptScenario?.visitorInputs.some((i) => i.mode === 'voice')).toBe(true);
  });
