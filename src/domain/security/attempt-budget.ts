 ...
 *
 * 🔴 **ただしこの予算は「1 人」ではなく「1 発信元」に効く（レビュー 5 周目 MINOR-7）。**
 * 鍵は XFF の末尾＝ NAT 越しなら**拠点で 1 つ**なので、同一拠点の複数端末が予算を共有する。
 * 設営時に PIN を取り違えたまま 3 台 × 4 回打つと 10 回に届き、**その拠点の初回認可が
 * 最大 10 分閉じる**（稼働中の端末は 30 日 cookie で無影響。復旧は admin → エンロール発行）。
 * 値を動かすなら別増分で —— ここでは**根拠を実態に合わせて書くに留める**。
 *
