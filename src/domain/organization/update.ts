...
> & {
  /**
   * 親組織。`null` は「親を外してトップレベルへ戻す」。
   *
   * `undefined`（キー自体が無い）は「触らない」なので、**両者を型で区別する**。
   * 同一視すると「親を外したつもりが何も起きない」という無音の失敗になる。
   */
 ...
 * ## 未知のキーは拒否する（無視しない）
 *
 * `id` / `tenantId` を書き換えられると別テナントの組織を作れてしまう。**黙って無視すると
 * 「送ったのに効かない」**という、運用者から見て最も分かりにくい失敗になるので、明示的に拒否する。
 *
 * `parentId` はここでは**形だけ**を見る。循環・深度上限・tenant/site 境界の判定は
 * 他の組織との関係が要るので、純粋な入力検証では決められない（`canSetParent` が行う）。
 *
 * ## 空の公開表示名を通さない
 *
...
export function validateOrganizationUnitPatch(input: unknown): Result<OrganizationUnitPatch> {
  ...
  if ('parentId' in o) {
    // `null`（外す）と文字列（設定）だけを受ける。空文字は id として不正。
    if (o.parentId !== null && (typeof o.parentId !== 'string' || o.parentId.trim() === '')) {
      return err('parentId must be a non-empty string or null');
    }
    patch.parentId = o.parentId === null ? null : o.parentId.trim();
  }
