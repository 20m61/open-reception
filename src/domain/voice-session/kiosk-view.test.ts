...
describe('voiceKioskReducer (#364 kiosk 配線 / #361 復唱 UI の純状態機械)', () => {
  ...
  it('復唱イベントに kind="department" が付くと readbackKind へ写す（部署復唱テンプレート分離, #361 申し送り）', () => {
    const heard = run([
      { type: 'activate' },
      { type: 'listenStart' },
      { type: 'heardNeedsConfirmation', displayName: '営業部', reason: 'low_stt_confidence', kind: 'department' },
    ]);
    expect(heard.mode).toBe('readback');
    expect(heard.readbackName).toBe('営業部');
    expect(heard.readbackKind).toBe('department');

    // 確定/聞き直しで readbackKind もクリアされる（他の復唱情報と同じライフサイクル）
    expect(voiceKioskReducer(heard, { type: 'confirmYes' }).readbackKind).toBeUndefined();
    expect(voiceKioskReducer(heard, { type: 'confirmNo' }).readbackKind).toBeUndefined();
  });

  it('kind 省略時（従来イベント）は readbackKind を持たない（後方互換）', () => {
    const heard = run([
      { type: 'activate' },
      { type: 'listenStart' },
      { type: 'heardNeedsConfirmation', displayName: '佐藤', reason: 'low_stt_confidence' },
    ]);
    expect(heard.readbackKind).toBeUndefined();
  });
...
describe('captionKeyFor（字幕/インジケータの意味論キー導出、PII を含まない）', () => {
  ...
  it('readback は readbackKind="department" のとき部署用テンプレートキーを返す（#361 申し送り: 「◯◯様」が部署名に付く不自然さの解消）', () => {
    expect(captionKeyFor({ mode: 'readback', readbackKind: 'department' })).toBe(
      'voice.readback.confirmDepartment',
    );
  });

  it('readback は readbackKind="staff"／未指定のとき従来どおり担当者用テンプレートキーを返す', () => {
    expect(captionKeyFor({ mode: 'readback', readbackKind: 'staff' })).toBe('voice.readback.confirmTarget');
    expect(captionKeyFor({ mode: 'readback' })).toBe('voice.readback.confirmTarget');
  });
});
