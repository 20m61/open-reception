 ...
 * 固定テンプレが行う。実測が変わったら**行を足すか日付を直す** —— 散文を書き換えない。
 *
 * 🔴 **守れている範囲を正確に言う（#728 レビュー B-2）。** データ化したのは
 * **観測の行だけ**で、節の残りは自由文のまま。#728 が挙げる 4 クラスのうち、
 * 実測で red になるのは **R3 の一部（リテラル `403` を含み、かつ日付も `#N` も
 * 同居していない文）だけ**である:
 *
 *  - **R1**（`403` と書かない断定 = 「例外なく拒否されます」）… 全スロットで green。
 *    検出規則が `403` のリテラルを必要条件にしているため、入口で除外される
 *  - **R2**（観測時点と**同居させた**断定 = 「2026-08-18 に確認したとおり、あなたの
 *    環境でも 403 です」）… 全スロットで green。規則の通過条件が「日付か `#N` の同居」
...
export function buildDelegationPrompt(
   ...
   *
   * 🔴 **省略は無音なので、配線はテストで縛る。** 既定値があるぶん、呼び出しから
   * 第 3 引数が落ちても「古い観測」の表示だけが黙って消える —— 陳腐化の可視化は
   * #710 の再発検知そのものなので、`delegation-prompt.test.ts` が
   * 「古い日付を渡したら本文に出る」を固定している（レビュー M-3。以前は変異させても
   * 全テストが green だった）。
   */
  today = '',
): string {
