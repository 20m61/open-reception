 ...
 *
 * 🔴 **窓が塞いだのは「離れた場所のしるし」だけ。** しるしをコマンドの**直後**に置いた
 * 再断定（`\`gh pr create\` は 403 が解消したので使う` / `\`gh pr merge\` は実測では
 * 通ったので使う`）は**依然として通る**（レビューの実測で 6 例）。塞いだのではなく
 * コストを上げただけである。
 *
 * 逆向きの誤りもある: 窓が狭いので、自然な禁止文まで拒否する
 * （`\`gh pr create\` および \`gh pr merge\` は使わないこと` / `\`gh pr create\` は、
 * 使わないこと` / 長いフラグ列で 16 文字が埋まる形 / `使用しないでください` のように
 * `ALLOWED_ROLE` に無い語彙）。**この PR 自身の summary もいくつか落ちる。**
 * 恒久策は「しるしの不在」ではなく「指示動詞の存在」で判定すること（後続 issue）。
 */
