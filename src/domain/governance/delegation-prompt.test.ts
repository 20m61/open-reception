...
describe('buildDelegationPrompt', () => {
  ...
  it.each(['merge', 'pr'] as const)('🔴 stopAfter: %s で REST 経路の指示が無条件に残る', (stopAfter) => {
    ...
    //
    // ⚠ **これは生成器が所有する文言に対する不変条件。** spec 由来の自由文
    // （`extraVerification` など）の `可能なら` は #729 で **warn 止まり**にしてあり、
    // 本文に載る（下の #729 の節がそれを固定している）。`BASE` にはそれが無いので
    // このアサーションは赤くならないが、**「本文に緩和語彙が絶対に出ない」ではない**。
