 ...
 * 必ずこの 2 つの間に入る。
 *
 * 🔴 **名前付きゾーンの範囲（-12〜+14）で取らない。** 名前付きの範囲で近似すると、その外側の
 * 拠点で「確定」と報告した state が実際には反転する。
 *
 * ## 書き込み側は塞いだが、ここはまだ狭められない (#800)
 *
 * `isValidTimeZone` は**名前付きゾーンだけを受理する**ようになった（`src/domain/operating-policy/tz.ts`。
 * それまでは `Intl.DateTimeFormat` が通るかだけを見ており、Node 22 / ES2024 は
 * **生のオフセット文字列（±23:59）も受理する** ―― 実測で `timezone: '-23:59'` が保存できた）。
 *
 * **それでもここを -12〜+14 へ狭めてはいけない。** 塞いだのは**これから**の書き込みだけで、
 * 既に保存されている値は変わらないからである。読み側は `Intl` が受理するので**壊れず、
 * 静かに残る**。範囲は「保存されうる値」ではなく「**保存されている値**」で取る必要がある。
 *
 * 狭めてよくなる条件は 2 つとも満たしたとき:
 *
 * 1. 実データの棚卸しで生オフセットが 1 件も無いことを確かめる（本番 DynamoDB の読み取りが要る）
 * 2. 見つかった分を名前付きへ移行する
 *
 * リポジトリ側は既に清浄であることを確認済み（2026-08-27・2 通り）: 生オフセットのリテラルは
 * **本ファイルのこの説明文以外に 1 件も無く**、seed / fixture も書いていない。残る侵入経路は
 * **管理画面から人が保存した分だけ**である。
 */
