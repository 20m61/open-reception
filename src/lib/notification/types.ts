/**
 * 呼び出し先・通知ルート設定のドメイン型 (issue #88, increment 1)。
 *
 * 受付後に「誰へ・どの順番で・どの手段で」通知するかをテナント/サイト境界の中で管理する。
 * 既存 src/server/notification/types.ts の通知実行ドメイン（NotificationTarget 等）とは責務が
 * 異なり、本モジュールは **設定（ルート定義）** を扱う。実行サブシステムは将来この設定を
 * 解決して NotificationRequest を組み立てる想定（docs/call-route-config-design.md）。
 *
