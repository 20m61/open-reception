/**
 * 通知ルート管理 API のリクエスト解釈ヘルパ (issue #88, increment 1)。
 *
 * - actor の実解決は中央モジュール @/lib/auth/actor に集約済みのものを re-export する。
 * - tenantId（必須）・siteId（任意）の取り出しと ServiceResult → HTTP 変換。
 * 認可そのものは #80 純関数（canAccessSite）に委譲する。
 */
import { NextResponse } from 'next/server';
import { asSiteId, asTenantId, type SiteId, type TenantId } from '@/domain/tenant/types';
import type { ServiceResult } from './call-route-service';
...
export function readRouteScope(
...
export function callRouteResponse<T>(result: ServiceResult<T>, successStatus = 200): NextResponse {
