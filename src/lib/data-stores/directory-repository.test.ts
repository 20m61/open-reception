...
describe('DataBackedDirectoryRepository (#274 ④)', () => {
  it('seed が memory backend に投入され list で返る（enabled フィルタは呼び出し側の責務）', async () => {
    const repo = makeRepo();
    expect((await repo.listDepartments(T)).map((d) => d.id).sort()).toEqual(['dept-1', 'dept-2']);
    expect((await repo.listStaff(T)).map((s) => s.id)).toEqual(['staff-1']);
  });

  it('putDepartment → getDepartment が round-trip する', async () => {
    const repo = makeRepo(false);
    await repo.putDepartment(T, { id: 'd1', name: '営業', displayOrder: 1, enabled: true });
    expect(await repo.getDepartment(T, 'd1')).toMatchObject({ id: 'd1', name: '営業' });
    expect(await repo.getDepartment(T, 'nope')).toBeUndefined();
  });

  it('putStaff → getStaff が round-trip し、同一 id は上書きされる', async () => {
    const repo = makeRepo();
    const cur = await repo.getStaff(T, 'staff-1');
    await repo.putStaff(T, { ...cur!, available: false });
    expect((await repo.getStaff(T, 'staff-1'))?.available).toBe(false);
  });

  it('reset で部署・担当者の両方が seed 状態へ戻る（テスト導線）', async () => {
    const repo = makeRepo();
    await repo.putDepartment(T, { id: 'extra', name: '追加', displayOrder: 9, enabled: true });
    const staff = await repo.getStaff(T, 'staff-1');
    await repo.putStaff(T, { ...staff!, displayName: '変更 済' });
    await repo.reset(T);
    expect((await repo.listDepartments(T)).map((d) => d.id).sort()).toEqual(['dept-1', 'dept-2']);
    expect((await repo.getStaff(T, 'staff-1'))?.displayName).toBe('佐藤 太郎');
  });
});
