...
describe('voice-store (#28)', () => {
  it('既定では TTS/STT とも無効（テキスト主導）', async () => {
    const v = await getVoiceSettings(T);
  ...
  it('案内文言を更新できる', async () => {
    const v = await updateVoiceSettings(T, { guidanceIdle: 'いらっしゃいませ' });
    expect(v.guidanceIdle).toBe('いらっしゃいませ');
  });

  it('話速・音量を範囲内にクランプする', async () => {
    const v = await updateVoiceSettings(T, { rate: 5, volume: 9 });
    expect(v.rate).toBe(2);
    expect(v.volume).toBe(1);
    const v2 = await updateVoiceSettings(T, { rate: 0, volume: -1 });
    expect(v2.rate).toBe(0.5);
    expect(v2.volume).toBe(0);
  });

  it('provider は browser/none のみ受け付ける', async () => {
    const v = await updateVoiceSettings(T, { ttsProvider: 'invalid' });
    expect(v.ttsProvider).toBe('browser');
    const v2 = await updateVoiceSettings(T, { ttsProvider: 'none' });
    expect(v2.ttsProvider).toBe('none');
  });

  it('プライバシー通知の要約文言を上書きできる（既定は未設定, #314）', async () => {
    const before = await getVoiceSettings(T);
    expect(before.privacyNotice).toBeUndefined();

    const v = await updateVoiceSettings(T, { privacyNotice: 'カスタム通知文言です' });
    expect(v.privacyNotice).toBe('カスタム通知文言です');

    const persisted = await getVoiceSettings(T);
    expect(persisted.privacyNotice).toBe('カスタム通知文言です');
  });

  it('プライバシー通知の上書きを空文字にすると未設定へ戻る', async () => {
    await updateVoiceSettings(T, { privacyNotice: 'カスタム通知文言です' });
    const v = await updateVoiceSettings(T, { privacyNotice: '   ' });
  ...
  it('呼び出し中の段階的ケアのしきい値・文言を上書きできる（既定は未設定, #323）', async () => {
    ...
    const v = await updateVoiceSettings(T, {
  ...
  it('段階的ケアのしきい値は 0/負値/NaN を無視する（既存値を保つ）', async () => {
    await updateVoiceSettings(T, { callingStageWaitingAfterMs: 5000 });
    const v = await updateVoiceSettings(T, { callingStageWaitingAfterMs: -1 });
    expect(v.callingStageWaitingAfterMs).toBe(5000);
    const v2 = await updateVoiceSettings(T, { callingStageWaitingAfterMs: NaN });
    expect(v2.callingStageWaitingAfterMs).toBe(5000);
  });

  it('段階的ケアの文言上書きを空文字にすると未設定へ戻る', async () => {
    await updateVoiceSettings(T, { guidanceCallingNotice: 'カスタム予告文言' });
    const v = await updateVoiceSettings(T, { guidanceCallingNotice: '   ' });
  ...
  it('ワンタップ満足度フィードバック収集の有効/無効を切替できる（既定は未設定=有効扱い, #320）', async () => {
    ...
    const off = await updateVoiceSettings(T, { feedbackEnabled: false });
    expect(off.feedbackEnabled).toBe(false);
    const persisted = await getVoiceSettings(T);
    expect(persisted.feedbackEnabled).toBe(false);

    const on = await updateVoiceSettings(T, { feedbackEnabled: true });
  ...
  it('アクセシビリティ支援モードの有効/無効をテナント設定できる（既定は未設定=全モード有効扱い, #321）', async () => {
    ...
    const updated = await updateVoiceSettings(T, {
  ...
  it('アクセシビリティ支援モードの不正値は既定=有効へ補正する（#321）', async () => {
    const v = await updateVoiceSettings(T, { a11yModesEnabled: { largeText: 'no' } });
...
describe('voice-store のテナント分離 (#419)', () => {
  it('別テナントの設定は混ざらない', async () => {
    await __resetVoice('acme');
    await updateVoiceSettings(T, { voiceId: 'default-voice' });
    await updateVoiceSettings('acme', { voiceId: 'acme-voice' });

    expect((await getVoiceSettings(T)).voiceId).toBe('default-voice');
    expect((await getVoiceSettings('acme')).voiceId).toBe('acme-voice');
  });
});
