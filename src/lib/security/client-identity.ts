 ...
 * 値そのものを保存する必要が無い。
 *
 * 🔴 **「PII が消える」とは書かない（レビュー 5 周目 MINOR-1）。** salt が秘密からの
 * **決定的導出**になった時点で、**その秘密を持つ者は候補 IP を当てて照合できる**
 * （IPv4 は空間が小さい）。主張できるのは**「保存値そのものは PII ではない」**までである。
 * `docs/persistence-design.md` §4.4 と**同じ表現に揃えてある** ——
 * ここだけ強い言い方が残ると、次に読む人が保持期間やエクスポートの扱いを緩める。
 *
