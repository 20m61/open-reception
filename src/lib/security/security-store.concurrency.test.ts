...
describe('セキュリティ設定の同時更新 (#1158)', () => {
   ...
   * **両方が成功と返り、最終値は解除**になった（この fixture で実測したのは「投入が先」の順序）。
   * 投入した運用者の画面は「有効にしました」のまま停止が黙って外れる。当て直すのは投入だけにし、
   * 解除は 1 回だけ試して負けたら競合にする。
   *
   * 🔴 **弁別力があるのは「投入が先」だけ**（closure check で、修正を戻すとこちらだけが赤になる
   *    ことを確認）。「解除が先」は危険な交差ではなく、投入が当て直しで勝つことを見る**回帰の下界**
   *    として置いている（当て直しそのものを壊す変異はこちらでも落ちる）。
   */
  it.each([
    ['投入が先', [true, false]],
    ['解除が先', [false, true]],
