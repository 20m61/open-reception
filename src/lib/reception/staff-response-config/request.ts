/**
 * 担当者応答設定 API のリクエスト解釈ヘルパ (issue #99 increment 2)。
 *
 * - actor の実解決は中央モジュール @/lib/auth/actor（resolveAdminActor）を re-export する。
 * - tenantId/siteId（いずれも必須）の取り出しと ServiceResult → HTTP 変換。
...
/** tenantId・siteId（いずれも必須）をクエリ or ボディから取り出す。 */
export function readConfigScope(
...
export function configResponse<T>(result: ServiceResult<T>, successStatus = 200): NextResponse {
