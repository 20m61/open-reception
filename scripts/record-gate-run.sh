...
# --- 外側ループ（規約改善）が回っているかも、同じ入口で点検する (#424) ---
#
# 内側ループ（Issue → PR → マージ）の記録が健全でも、**規約そのものが更新されなければ
# 同じ失敗を踏み続ける**。外側ループ（`/loop-retro`）は定期実行なので、週次運用の入口が
# 唯一の自動経路である。ここに置かないと、`loop-retro.ts` は #656 と同じ
# 「作ったが誰も走らせない検出器」になる。
#
# 上と同じ理由で **`--report`（exit 0）** で呼び、終了コードには混ぜない。
echo ""
echo "▶ 外側ループ（規約改善）を点検します（報告のみ・終了コードは変えません）"
npm run --silent loop:retro -- --report
RETRO_STATUS=$?
if [[ "${RETRO_STATUS}" -ne 0 ]]; then
  echo "⚠️  外側ループを点検できませんでした（exit ${RETRO_STATUS}）。指摘の有無は不明です。" >&2
fi
