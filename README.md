...
`/admin/*` と `/api/admin/*` は管理セッション必須です（`src/proxy.ts`＝旧 middleware で保護。Next 16 で `proxy` へ改称）。未認証は `/admin/login` へリダイレクト、管理 API は 401 を返します。受付端末（kiosk）からは管理画面/API にアクセスできません。`/api/kiosk/*` は公開です。
...
| --- | --- |
| `npm run dev` | 開発サーバ起動 |
| `npm run build` | 本番ビルド（Next.js 16 / Turbopack） |
| `npm run build:open-next` | AWS デプロイ用成果物 `.open-next/` を生成（OpenNext） |
| `npm run typecheck` | 型チェック (`tsc --noEmit`) |
| `npm run lint` | ESLint（Flat Config / ESLint CLI。Next 16 で `next lint` 廃止） |
...
- [AWS デプロイ手順（OpenNext + CDK）](./docs/deploy-aws.md)
