...
ゲートが赤いとき、**コードを疑う前に資源を見る**。見るのは 3 つ:

0. **作業ツリーを誰かが触っていないか** … ゲートと**同じツリー**でレビュー用サブエージェントを
   並走させると、変異検証がファイルを書き戻すたび mtime が進み、
   `OpenNext build artifacts are older than src/` で **infra が丸ごと落ちる**（2026-08-26 に実際に
   起きた。`git status` は差分ゼロのまま＝内容ではなく mtime だけが動く）。**変異を測る
   エージェントは worktree で隔離するか、ゲートと直列に回す**
