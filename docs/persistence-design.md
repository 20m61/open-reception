...
### 4.2 試行予算（`auth-attempts`）の保持（#1021 AC4）

認証の試行回数制限（`/api/kiosk/authorize` / `/api/admin/login`）が使うコレクション。

| 項目 | 内容 |
| --- | --- |
| 保持期間 | **2 時間**（`ttlSeconds`。窓を開き直すたびに延長する） |
| 保存する値 | 窓の開始時刻・窓の中で入場した試行の回数 **のみ** |
| 鍵 | `<経路>#ip:<sha256(salt + viewer IP)>` または `<経路>#global` |

🔴 **生の IP は保存しない。** 予算の鍵は「同じ発信元か」を判定できれば足りるので、
値そのものは要らない。`ATTEMPT_KEY_SALT`（サーバ側 salt）を混ぜた不可逆ハッシュにする
—— IPv4 は空間が小さく、salt 無しの `sha256(ip)` は総当たりで逆引きできるためである。
