...
`/api/admin/login` は射程外である（独立レビュー 3 周目で撤回。#1167 で設計し直す）。
...
🔴 **salt は専用の設定値ではなく、`KIOSK_ENROLLMENT_SECRET` から HKDF で導出する**
（`client-identity.ts`）。この方式は 2 度替えていて、**どちらの失敗も残す**:

1. 最初は `ATTEMPT_KEY_SALT` という専用 env にしたが、**`infra/` にも `docs/deploy-aws.md` にも
   配線が無く**、実デプロイではリポジトリに平文で置かれた dev 既定値が使われていた ——
   公開既定値の salt は salt ではないので、「逆引きできない」が**配備の現実で偽**だった
2. 次にプロセス起動ごとの乱数にした。設定は要らなくなったが、**鍵がプロセス間で安定しなくなり**、
   Lambda の同時実行ごとに同じ IP が別の一次鍵へ割れた ——
   **1 本の IP を並列に投げるだけでサイト全体の初回認可を閉じられた**（下記の上界が偽になった）

`KIOSK_ENROLLMENT_SECRET` は `failClosed: true`（実デプロイで未設定なら起動が落ちる）で、
本文書と同じく `docs/deploy-aws.md` が Secrets Manager の必須項目として名指ししている ——
**既に配線されている**。だから公開既定値に落ちず、かつプロセス間で安定する。
署名鍵をそのまま使わず専用ラベルの HKDF で導出するので、**保存されたハッシュから署名鍵は導けない**。

この導出により、`attempt-store.ts` が主張する上界 ——
**1 つの発信元が二次（cap）へ入れられるのは高々 `perOrigin.budget` 回** —— が本番でも成り立つ。
鍵空間の増大と書き込み増幅の監視は #1168。
...
🔴 **「復元できない」の正確な意味を書く。** 鍵は不可逆ハッシュなので、保存された値だけからは
IP を引けない。ただし **salt は `KIOSK_ENROLLMENT_SECRET` から決定的に導出される**ので、
**その秘密を持つ者は候補 IP を当てて照合できる**（IPv4 は空間が小さい）。
salt をプロセス起動ごとの乱数にしていた版では「プロセスが終われば照合もできない」と書けたが、
その版は一次予算が実行環境ごとに割れる欠陥を持っていた（上記 2）ので採らない。
したがってここで主張できるのは **「保存値そのものは PII ではない」** までで、
**「誰にも復元できない」ではない**。秘密の管理は署名鍵と同じ扱い（Secrets Manager）。
