...
- **通話結果の主 CTA（#324-5）**: `connected` は「担当者がまいります／操作は不要です」を明示し、
  ...
  文言（操作不要）と挙動を一致させるため、`connected` を `INACTIVITY_RESET_STATES` に含めて
  **無操作で待機へ自動復帰**させる（画面にメディアは無い＝ビデオは `calling` の `KioskCallView` が持つ）。
  離席した来訪者のセッション（PII）を残して端末をブロックしないための整合であり、待機中の来訪者は
  既存の警告カウントダウン（#125）で「続ける」を押せば延長できる。connected は待ちが長くなり得るため
  `CONNECTED_INACTIVITY_RESET_MS`（既定 120s）で他画面（60s）より長めに取る。
