...
3. **音声とタッチで同じ目的を達成できる** — 一方が失敗してももう一方へ自然に切り替えられる。
4. **システム状態を沈黙させない** — 聞き取り中、認識中、確認中、発信中、接続中、失敗を明示する。
5. **途中経過を失わない** — 聞き返し、通信失敗、担当者不在でも最初からやり直させない。
6. **人につながる逃げ道を残す** — 自動受付を完遂できない場合に代表窓口や有人支援へ移れる。
7. **公共空間のプライバシーを守る** — 個人情報、発話内容、担当者情報を必要以上に表示・読み上げない。
...
- **Input mode**: touch / voice / QR を理由に Stage 全体を別 UI へ置換しない。入力手段だけを差し替える。
- **Fallback**: VRM、TTS、STT が無くても同じ情報構造を保ち、フォーム主体の別体験へ退行させない。
- **Motion**: 全面 slide / 全面 fade / blank frame を進行の主表現にしない。局所 transition は連続性を補助する場合だけ使い、reduced motion でも意味が成立すること。
- **Accessibility**: 視覚連続性のために heading、focus、live region 等の semantics を弱めない。

signage / attract から reception へ入るモード境界、または別 journey である checkout の入口は例外に
なり得る。ただし各 journey に入った後は同じ原則を適用する。詳細な実装・Evidence は #779 / #1055 / #782。
...
ただし**原則 3 の等価性が要求するのは音声とタッチの 2 つだけ**で、`scanning` は加速手段。
