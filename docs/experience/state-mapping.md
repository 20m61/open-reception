...
| 音声対話 | `src/domain/voice-session/kiosk-view.ts` | `inactive` / `idle` / `listening` / `readback` / `speaking` / `ducked` / `fallback` / `unavailable`（8） |
...
| `person_unavailable` | `ReceptionState.timeout` ＋ `VoiceKioskMode.unavailable`（#803） | **対応**（名前が違う）。**経路が 2 本ある** —— 呼び出して応答が無い（`timeout`）と、不在の相手を音声で名指しされた（`unavailable`）。来訪者から見れば「その人には繋がらない」という同じ事実なので同じ体験状態へ写す |
...
3 状態で `voice` を allowed input と**宣言**していたが、実際に音声経路が在るのは
`selectingTarget` だけだった。目的選択・氏名入力・発信確認はタッチが要る。
**宣言と実装が 3 分の 2 で食い違っていた**（ADR 0007 の訂正節）。

> **決着（宣言側のみ）**: この件は 2 つに分かれる。
> **(a) 宣言を実態に合わせる**は仕様判断を含まない事実の訂正なので実施済み。
> **(b) 音声入力を実装する**は Journey の意味に触れるため**要ユーザー確認のまま**。
>
> (a) の調査で、食い違いは `voice` だけでなく `text` にも及ぶことが判明した
> （`PurposeView` はボタンのみでテキスト入力もチャットも無い）。現在の宣言は:
>
> | 局面 | 宣言（現在） | 根拠 |
> | --- | --- | --- |
> | `selectingPurpose` | `touch` | `PurposeView` はカードボタンのみ |
> | `selectingTarget` | `touch` / `voice` / `text` | 検索欄 + `onResolved`（音声で相手を確定） |
> | `inputVisitorInfo` | `touch` / `text` | 氏名フォーム。音声で `SUBMIT_VISITOR_INFO` を生む経路は無い |
>
> `ui-contract.test.ts` が上表を固定し、`voice` を宣言できるのは `selectingTarget` だけで
> あることをメタテストで担保する。**実装が追いついたときに宣言を足す**（宣言を先に置かない）。
>
> **原則 2 が満たされていない事実は変わらない**。腕が塞がっている来訪者・視覚に頼れない
> 来訪者・騒音下では音声だけで受付を完遂できない。これは (b) の判断待ち。

**D. 別の状態機械（統合するか否かが仕様判断）**: J-OR-03 QR 受付（`CheckinFlow`）。
→ **ADR 0008 で「今は統合しない」**。統合の是非を先に決めるのではなく**並存のコストを実測**した
結果、コストは「**片方に入れた修正がもう片方へ伝播しないこと**」だった。実害が 1 件見つかっている
（第 36 wave で通常受付に入れた「通信断とサーバ失敗を分ける」修正が QR 側に無く、403 / 400 /
その他をすべて「通信に失敗しました」と表示していた）。是正済み。**統合の是非そのものは
要ユーザー確認のまま**（J-OR-03 を同一タイムラインに載せるかは体験設計の判断）。
