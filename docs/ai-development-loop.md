...
出力は **3 状態**ある。「触れていない」と「測れていない」を混ぜないため (#709):

| 出力 | 意味 |
| --- | --- |
| `停止境界に触れていません（人間承認は不要）` | **集めきれたうえで**当たりが無かった |
| `⚠ 人間承認が必要な変更に触れています` | 当たった（根拠パスつき） |
| `⚠ 停止境界の判定はできていません` | **変更パスを集めきれなかった**（起点が無い / git が失敗した）。理由と失敗したコマンドを名指しする。**集まった範囲に当たりがあればそれも出す**（保留は「根拠が無い」ではなく「範囲が不完全」） |
...
既にガードを持っており、`change-risk` 側だけ無かった。**空集合を「問題なし」と読まない。**
...
- [x] 定期評価レポート生成コマンド … `npm run evaluate:gate-runs`
      ...
      週次を超えて止まっている / **記録の途中に穴が空いている**（`record_gap`、#656）/
      直近が FAIL / FAIL に issue 参照が無い / SKIP が記録されている
      （`--strict` 下では出ないはず＝strict 未使用か環境劣化）。
      **穴の検査は「止まっている」の検査では代替できない** — 後者は直近 1 件の経過日数しか
      見ないので、途中の 1 回が main に載らなくても**次の回が載った時点で永久に見えなくなる**
      （#656 で実際に起きた）。穴は隣接する `full` 記録の間隔で捕まえ、抜けた回の行を
      追記すれば消える（並び順は日時で決まるので後から挿せる）。
      あわせて**リモートに在るのに PR が 1 つも無いブランチ**を `orphan_branch` で指摘する
      （#656 の直接原因＝ push したが PR が作られなかった）。**検査できなかった場合は
      `branch_check_unverified` を出して緑にしない** — 空の結果を「問題なし」と読むのは、
      この検査が塞ごうとしている失敗そのもの。
