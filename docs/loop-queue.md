...
| **#424** | ai loop | **増分 1 完了**（第 78 wave: `docs/ai-development-loop.md` = 10 フェーズと実在する仕組みの対応表・observe できるものの出どころ・fitness functions 棚卸し・停止境界・段階公開の現物・**受入条件 6 件に対する正直な棚卸し**）。#426 の必須内容も同時に満たす。**残**: Issue テンプレート / PR 本文への仮説・rollback・計測の必須化 / schema diff チェック / change-risk classifier / 追跡 ID / kill switch・変更量上限 / 定期評価レポート / 元 issue への計測書き戻し。**受入条件は 6 件中 0 件が充足**（文書だけでは満たせない） | ローカル可（継続） |
...
| **#426** | docs | **作成対象 3 件すべて存在**（第 78 wave）: `docs/product-integration-plan.md`（第 16 wave）/ `docs/adr/README.md`（同）/ `docs/ai-development-loop.md`（第 78 wave）。**残**: 受入条件「各 PR が基準文書または ADR を参照する」「設計変更時に文書更新を要求する PR チェック項目がある」= PR テンプレートへの追加（#424 増分 2 と同一作業） | ローカル可（#424 増分 2 と同時） |
