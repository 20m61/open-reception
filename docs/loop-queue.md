...
| **#424** | ai loop | **増分 1〜2 完了**（第 78〜79 wave）: `docs/ai-development-loop.md`（10 フェーズと実在する仕組みの対応・fitness functions 棚卸し・停止境界・受入条件の正直な棚卸し）＋ Issue テンプレート 2 種（`improvement` / `ai-proposal`。観測・仮説・**反証条件**・計測を必須欄に）＋ PR テンプレート拡充（仮説 / 計測・成功停止条件 / リスクと戻し方 / 基準文書・ADR 参照 / 人間承認が必要な変更）。**残**: schema diff チェック / change-risk classifier / 追跡 ID / kill switch・変更量上限 / 定期評価レポート / プロンプト・ツール実行の監査ログ / **元 issue への計測書き戻し**。受入条件は 6 件中 0 件が充足（自動化が要る） | ローカル可（継続） |
