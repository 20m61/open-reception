...
> **直近の引き継ぎは `docs/handoff-2026-08-28.md`。** 2026-08-26 の分（`docs/handoff-2026-08-26.md`）は
...
| 順 | Issue | なぜこの順か | 着手可否 |
| --- | --- | --- | --- |
| 1 | **#826** | `kiosk-calling-stage` が **5 回中 1 回 flaky**（2026-08-28 実測）。retry で緑になるので `--full` を素通りする ＝ **唯一の品質担保に確率的な穴**が開いている。他の何を直すより先に、ゲートが信用できる状態にする | ローカル可 |
| 2 | **#813** | lint の warning が `--max-warnings` 不在でゲートを素通りする。2026-08-28 の実測で **86 warnings**。#826 と同じ「ゲートが見た目より弱い」族。**棚卸しが前提**なので #826 の後 | ローカル可 |
| 3 | **#818** | 全 spec がちょうど 1 つの project に載ることの機械化。#787 で「project を分けても隔離にならない」を踏んだ再発防止 | ローカル可 |
| 4 | **#817** → **#816** → **#815** | #787 の周回で実測から分離したもの。#817 は**営業時間外に全群がその形になる**ので 3 つの中では先 | ローカル可 |
