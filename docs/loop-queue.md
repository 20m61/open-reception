...
## 現在地（2026-07-27 更新・第 27 wave 消化後）
...
次は **#422 increment 3（画面コンポーネントの分割）**。詳細は第 16〜27 wave の各節。
...
**次に着手する候補（2026-07-27 更新・第 27 wave 消化後）**: **#422 increment 3
（画面コンポーネントの分割）**。副作用がフックへ出たので、`renderScreen` の巨大な引数列
（20 個超）と各 View の切り出しに進める。
