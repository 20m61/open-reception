...
> **これで `ConversationTurnView` の消費者ゼロ導出が無くなり、#422 inc5 は一巡した。**
> 第 72 wave で受入条件 7 件を再マッピングし、唯一のギャップ（VRT の結果系）を #499 で埋めた。
> **#422 の受入条件はすべて充足。** 残るのは実機検証（#65）と linux VRT ベースライン（#480）。
