...
### 逃げ道バーが覆っている内容の提示（#816）

**AC は 2026-09-02 の `/issue-ac-mapping` で改訂済み**（「余白で塞ぐ」は原理的に効かないため、
「隠れているときだけ提示する」へ）。その改訂どおりに実装した。

- 判定は純関数 `src/domain/kiosk/escape-bar-occlusion.ts`。**バー上端**と**in-flow の兄弟の
  最下端**を実測して引き算する。`position: fixed` / `absolute` の兄弟は数えない ——
  `.kiosk-avatar-companion` は fixed で **viewport 左下**に居るため、数えると覆いゼロの画面でも
  出しっぱなしになる
- 提示は `EscapeBar` **自身**に持たせた。`KioskFlow` 側に置くと QR 受付（`CheckinFlow`）に
  入らない（`EscapeBar` の docstring が「バーを 2 つ実装すると片方だけ直る」と明記している）
- 判定は VRT ではなく**ヒットテスト**（`tests/e2e/kiosk-escape-bar-occlusion.spec.ts`）。
  スクリーンショットでは「少し切れている」としか見えず、覆いの有無を機械で区別できない
...
🔴 **末端でバーは下端に付かない。** `.screen` の下余白ぶん浮き、さらに本文終端はバー上端の
24px 上にある。「末端から少し戻せば覆いが始まる」は**成り立たない** —— 戻し量を決め打ちに
せず、覆いが始まるまで少しずつ戻して測ること。

#### 変異検証（純関数 11 種 + 配線 9 種）

純関数は 11/11 kill。**配線は 3 種が生存し、そのつどテストを足した**:

- 「バー上端ではなく**下端**で測る」… 深く潜っている画面ではどちらの基準でも「覆っている」に
  なるため、既存の上界テストが基準点を区別できていなかった。**バーの高さ未満の覆いを全部
  取りこぼす**変異で、まさに #816 が名指しした欠陥そのもの。覆いが始まった瞬間を捉える
  テストを足して kill
- 「本文のリサイズ観測をやめる」… 検索で一覧を縮める経路（#788 と同型）を踏んでいなかった。
  縦に余裕のある viewport で足して kill
- 🔴 「**外から渡された ref を無視して内部 ref を使う**」… これは**この周回の refactor が
  作った**穴である。React Compiler の `react-hooks/immutability` が「呼び出し側の ref へ
  代入する」コードを error にするため ref を 1 つに統一したのだが、統一を誤ると
  `KioskFlow` の `escapeBarRef` に node が入らず `--kiosk-chat-safe-bottom` が既定のまま残り、
  **チャット FAB が逃げ道バーに重なる**（#121 H1 の再発）。既存テストは 1 本も落ちなかった。
  バーの実測高さが持ち上げ量へ渡っていることを直接主張して kill

**「兄弟の走査を直前の 1 つだけに狭める」も一度は生存した。** `.kiosk-chat-slot` が
`:empty` で `display:none` になる局面（`idle` / `cancelled` / `completed`）では、直前の兄弟が
数えられず、その 1 つ前は fixed のアバターコンパニオン ——**本文は 3 つ前にある**。
完了画面（退館 QR で縦に長い）を縦の詰まった viewport で踏んで kill。

方式を替える（ref の持ち方を変える）たびに **9 種を全部当て直し**、先に kill していたものが
kill のままであることを確認した
（`.claude/rules/opus5-autonomous-loop.md`「方式を替えたら、前の方式が守っていた変異を当て直す」）。

⚠️ **`ro.observe(bar)`（バー自身のリサイズ観測）は最後まで生存した。** 到達可能な状態では
`document.body` の観測と区別が付かない。保険として残したが、**テストで縛れていないことを
コード内に明記**してある。

#### darwin VRT

閾値 0 で測って動いたのは `kiosk-landscape-purpose` と `kiosk-landscape-target` の **2 枚だけ**。
`kiosk-landscape-out-of-hours` も閾値 0 では落ちるが、差分は**mask 済みの日付**だけで
本変更とは無関係（spec が「唯一この 1 枚が許容値 0 で落ちる」と注記済み）なので**更新しない**。

**用件選択画面も 1080x810 で覆われていた**（監査も #816 本文も相手選択しか挙げていなかった）。
...
### platform の一覧の状態表示（#896・PR #947）

🔴 **検出器を先に書いたから、自分の検索式の取りこぼしが見つかった。** 直す対象を
`data?.X ?? []` で探して 6 箇所見つけ、そのつもりで直したあと、構造テストを走らせたら
**8 箇所の取りこぼし**が出た。`MaintenanceStatus` は `data?.summary.devices ?? []` と
**入れ子**なので検索式に掛かっていなかった（「その条件では見つからなかった」を「無い」と
読む型）。**先に検出器を書き、それを red にしてから直す**とこの型は自動的に潰れる。

**免除は「ファイル」ではなく「式」で持つ。** 表を 4 つ持つファイルをファイル単位で免除すると、
1 つ直しただけで残り 3 つが素通りする。さらに変異検証で、**免除の水増し**（`.map(` のような
広すぎる式を 1 行足すと全部が免除になる）が通ることが分かったので、
**ちょうど 1 つに当たること**を要求する形にした。

**閾値そのものを緩める変異は、テストでは捕まえられない。** 「表が 9 個以上実在する」という
下界の**数値を 0 に書き換える**変異は生存する。アサーション自身の改変なので、テストが
テストを検査しない限り検出できない（本リポジトリの全ての閾値に同じことが言える）。
**直せる穴ではなくレビューが担う範囲**として記録しておく。

### 🔴 同じ issue を 2 つのセッションが並行で実装した（#927）

2026-09-03、**#927 に対して PR #943（本セッション）と PR #944 が別々に作られた**。#943 が
先にマージされ、#944 は unmerged のまま閉じられている。成果物は失われていないが、
**片方の実装は丸ごと無駄になった**。

`CLAUDE.md` の並列規約は「同一ファイルを触らせない」までしか言っておらず、
...
> 🔴 **既存 6 件のうち実害があったのは 1 件**（`VrmAvatarViewer` の視線が iPad の回転に
> 追従しない）。**3 件は別の依存にマスクされていて現状は到達不能**で、マスクを外す変更で
> 真になる。当初「4 件が本物のバグ」と書いたのは、依存の欠落から実害を一段飛ばしに
> 断定した誤りで、独立レビューの指摘で訂正した。
> 方針と内訳は `docs/quality-gate.md`「lint の warning は素通りしない」。
> **`--max-warnings` の数値は減らしたら一緒に下げること。**
