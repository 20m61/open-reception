...
**次に着手する候補（2026-08-01 更新・第 113 wave 消化後）**

> **#554 は残り 2 画面**（`/admin/staff-response` と `/admin/demo`）。移行の型は完成していて、
> **`resolveScopeGate`（`src/components/admin/scope-gate.ts`）に載せるだけ**でよい:
> ページで `resolveAdminTenantId()` と `resolveDefaultScope()` をサーバ解決 → `useSiteScope` →
> `SiteScopeSelect`（`onRetry` は必須 prop なので忘れようがない）→ `<data>ScopeKey` で
> 載っているスコープを保持 → 門は **`canCreate`（一覧に依存しない書き込み）と
> `canMutate`（載っている行への操作）を分ける**。**独自 fetch を持つハンドラを数えて
> 全部に門を写すこと**（#562 の `reissue` が実例）。
>
> - `/admin/staff-response` … 既定値 `'default-site'` は正しいが固定で選べない。最も軽い。
> - `/admin/demo` … `siteId={String(tenantId)}` ＝ **拠点 ID にテナント ID を渡している**。
>   移行ではなく**まず調査**（何を意図していたのかが不明）。
>
> 次いで #557（浅い clone でのゲート base 誤解決）。
