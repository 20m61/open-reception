...
| #896 | platform の生 table・読込中と 0 件の描き分け（課題 06）。**2026-09-03 に AC2/3/4 と 横スクロールを消化**（PR #947）、続けて **AC1 の 13 表を `ui/DataTable` へ移行**（PR #965）。純粋なリファクタの見込みだったが**振る舞いは変わった** —— `AuditLogs` と `MaintenanceStatus` の 3 表は `{data && …length === 0}` の分岐で読込中に空の表を出していたので、寄せたことで「まだ読めていない」が 0 件から分離された。管理画面側 4 画面の同型欠陥は #966 |
