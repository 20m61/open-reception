...
> 🔴 **キューより先に「人にしか出来ない残件」を見る。** 下の 2 件は Issue の周回ではないので
...
`SignageManager` は `config.items.map` の TypeError で**画面ごと落ち**、
`OperatingHoursManager` は `applyPolicy(undefined)` で**フォームを黙って初期化**して
#367 の楽観ロックを外していた。述語（`src/domain/*/parse.ts`）を型の隣に置き、
load と save の両方で通す。

**この周回の主題は、レビューが 4・5 周目に見つけたものが「コードの欠陥」ではなく
**私の主張の反証**だったこと**である。BLOCKER は 5 周とも 0、MAJOR は **4 → 2 → 2 → 1 → 1**。

#### 🔴 「e2e では原理的に縛れない」と書いたが、誤りだった（4 周目）

3 周目に「`await res.json()` を跨いで拠点が変わる窓は e2e では**原理的に**作れない」と
判定し、それを **PR 本文と Issue #1013 に書いた**。根拠は「`route.fulfill` はヘッダと本文を
分割配送できない」で、これ自体は正しい。誤りは**そこから「e2e の限界」を結論したこと**である。

`page.addInitScript` で `Response.json()` の解決を保留すれば窓は決定的に作れる。しかも
**同じ idiom はこのリポジトリの `kiosk-calling-stage.spec.ts` / `kiosk-fallback.spec.ts` に
既にあった**。自分が使おうとした 1 つの道具（`route`）の限界を、層の限界と取り違えていた。

**教訓の候補**（`/loop-retro` へ）: 「調査の作法」は**否定的な結論ほど 2 通りで確かめよ**と
言っているが、それを**能力の否定**（「この層では作れない」）にも適用すること。
「その条件では見つからなかった」と「無い」の区別は、検索だけでなく**手段の探索**にも効く。
兆候は「リポジトリ内に前例が無いか探していない」ことだった。

#### 🔴 その反証を受けて足した e2e が、今度は空虚だった（5 周目）

4 周目の指摘に応えて追加した spec は `__release?.()` を使っており、**窓が開かなかったとき
無言で no-op** する。そして窓が開くかどうかは「前の拠点の応答が拠点切替の確定より前に
届くか」という**競争**である。レビューは応答順を入れ替えるだけで、**修正を戻したビルドで
このテストを緑にしてみせた**。

CLAUDE.md の「**下界**を併せて縛る」「既存の回帰テストが空虚に通るようになっていないか測る」が
言っている型そのものだが、**`?.()` という具体形は本書にもレビュー規約にも書かれていない**。
対策は「窓が開いたことを表明してから解放する」＋「解放の正の観測点を置く」で、
**レビューの exploit を再現して大声で落ちることを確認**してから閉じた。

🔴 **この段落は、最初は「CLAUDE.md が『`?.()` は空虚化の典型』と名指ししている」と
書かれていた（誤り）。** 出所を辿ると、`?.()` の一節は**私がレビュー依頼のプロンプトに
書いた言葉**である。レビューがそれを CLAUDE.md 由来として書き返し、私がその帰属をそのまま
記録へ写した ―― **自分の言葉が 1 往復して典拠に化けた**。指摘したのは PR #1014 に付いた
自動レビューで、`grep '?\.()'` を 1 回打てば分かることだった。

**教訓の候補**（`/loop-retro` へ）: 台帳は `/loop-retro` の入力なので、**帰属を伴う主張は
書く前に原典を引く**。とくに「規約に書いてある」という形の主張は、レビューやサブエージェントの
出力を経由した時点で**自分の言葉が戻ってきているだけ**の可能性がある。「調査の作法」が
検索条件について言っていることは、**典拠についても同じ**である。

#### 🔴 変異行列を「自分が書いたテスト」から導くと、穴がそのまま写る（3 周目）

3 周目に生存した 2 種の位置が、**私がテストを書かなかったフィールドと正確に一致**していた。
行列をコードから導いていなかった。`parse.ts` の `if` を機械的に列挙し直したら 31 種になり、
全部 kill された。同じ周で「配線を変異させていない」（1 周目）「等価変異を当てていた」
（2 周目・原因は変異設計ではなく**戻り値の型** `T | null` が 2 つの場合を畳むこと）も出た。

**自分の変異設計が 4 回外れた**ことになる。行列の網羅性は自己申告できない。

#### 縛れたもの / 縛れなかったもの

- 述語 31 種・配線 9 種すべて kill。`scope-gate` の不変条件は**総当たり 256 通り**で縛り、
  「拠点側の理由を優先する」という doc の約束も片側の含意として足した（null か否かだけでは
  `loadFailed` を `no-site` の前へ動かす変異が 18 本を素通りしていた）
- `PLAYWRIGHT_BASE_URL` 実行では H1 の 1 本が skip される（seed 由来の `branch-site` が
  実環境に無い）。実デプロイに対する縛りはゼロ

#### 切り出した Issue

- **#1012** 管理画面の「再試行」導線が、押した結果を返さない（7 画面共通）。失敗を繰り返しても
  DOM が 1 ビットも変わらず、フォーカスは `<body>` へ落ちる
- **#1013** `await res.json()` を跨いだ拠点切替が admin の残り 6 画面で無縛り。
  **4 周目の反証を受けてタイトルと本文を訂正した**（「原理的に縛れない」→「未実装」）
- **#1004** に第 2 増分（kiosk = 来訪者導線を優先）・第 3 増分（検出器と登録簿）を記録。
  この族は全体で **70 箇所**、admin だけで **26 箇所**残っている
...
`grep -n delete scripts/merge-pull-request.ts | head -20` がブロックされたこと。
...
### 🔴 7 周とも同じ型の BLOCKER が出た

毎周「**変更前が止めていた実行形が素通りする**」。うち 3 件は実際にプログラムが起動する
ことを実測で示された（`GIT_EXTERNAL_DIFF=` / `git --config-env=` / `rg --hostname-bin=`）。
1 件は**前周の指摘を直すために足した機構**が作った穴で、`npm run build >/tmp/b.log 2>&1;gh pr merge 997`
という日常的な形を素通しにしていた。

到達した原則は 3 つ（`docs/quality-gate.md` に履歴つき）:

1. **解釈しない** … シェルを解釈しようとするたび bash とずれた（`|&` → プロセス置換 →
   `&>` → 環境変数 → 改行 → 行継続 → 語中の `#`）
2. **数え上げない** … 「起動器を渡せる option 名」を列挙したら `rg --hostname-bin` が漏れた。
   道具も option も allowlist にし、知らないものは通さない
3. **機構を足さない** … 塞ぎにいった穴より、足した機構のほうが危なかった

🔴 **自分で当てた変異は毎周「全部 kill」だった。** 族ごとの見落としは独立レビューでしか
出ていない（9 → 4 → 3 → 4 → 3 → 3 → 3 族）。**「行列が全部 kill」は「穴が無い」を意味しない。**

### マージ判断はユーザーが下した

7 周目でも BLOCKER が出ており、`.claude/rules/opus5-autonomous-loop.md` の停止条件
（BLOCKER/MAJOR ゼロの周が 1 回出るまで）は**満たしていない**。「変更前より両方向とも厳密に
良い」ことを根拠に、ユーザー判断で現状マージした。残る素通りは **#998**。

### 同じ周回で始末したもの

- **#988 をクローズ**（AC 3 件とも PR #986 で充足していたことを実測で裏取り）
- **#985 にコメント**（AC が #986 の設計判断と衝突している。方針判断待ち。
  クラウドで `--pr` が原理的に完走しない件の対処案 3 つを整理した）
- **#998 を起票**（変更前から残る素通り 4 族）

### 🔴 環境: gitleaks / semgrep が入っていない状態で始まった

SessionStart の `gate-tooling` 報告が名指ししており、runbook の復旧手順
（`cloud-setup.sh` の該当部分を手で実行）でそのまま復旧できた。**復旧後は `--full` が
SKIP なしで通る**（14 ステップ全 PASS を 5 回）。#985 のコメントに実測を残した。

### 人手が必要な残務

- 🔴 **リモートブランチ `claude/handover-info-followup-lgu1c4` が消せていない。**
  `git push origin --delete` がクラウドから `the remote end hung up unexpectedly` で失敗する
  （2 回試行）。ローカル macOS から消すこと（`docs/handoff-2026-09-04.md` §4 と同型）
...
### 一覧の列ソートとページング・残り（#910・PR #959・クローズ済み）

🔴 **ブランチを切り忘れて `main` へ直接コミットし、古いブランチを push・マージした。**
PR #956 がマージしたのは #953 で既に入っていた内容で、**実質 no-op の重複マージ**。
`main` を確認せずに #910 をクローズしてしまい、**訂正して再オープン**したうえで
出し直した（PR #959）。

**「マージできた」は「入った」ではない。** `merge-pull-request.ts` は `merged=true` を
REST で確認するが、それは**その PR がマージされたこと**しか言わない —— 中身が意図した
差分かは別である。確実なのは**マージ後に `main` の現物を読む**ことだけ:

```
git show origin/main:<path> | grep -c <入るはずの印>
```

**同じブランチ名を 2 増分で使い回す**と起きやすい（`feat/...` を #953 と残りで共用した）。
増分ごとに新しい名前を切る。

🔴 **移行では testid を推測しない。** 手書きページ送りを共有部品へ寄せる際、testid の
接頭辞を**既存ソースから実測して**引き継いだ。推測していたら `reservation` を `rsv`、
`receptions` を `reception` と取り違えていた（**実際に両方とも推測と違った**）。
既存 e2e が 1 行も変わらず緑であること自体が、移行が振る舞いを変えていない証拠になる。

**例外の登録簿は、空になったら畳む**（残すと「ここに足せば通る」逃げ道になる）。
**畳んだら冒頭の注記も直す** —— 注記が「PENDING に残す」と言ったままだと検査と食い違う。
...
| #896 | platform の生 table・読込中と 0 件の描き分け（課題 06）。**2026-09-03 に AC2/3/4 と 横スクロールを消化**（PR #947）、続けて **AC1 の 13 表を `ui/DataTable` へ移行**（PR #965）。純粋なリファクタの見込みだったが**振る舞いは変わった** —— `AuditLogs` と `MaintenanceStatus` の 3 表は `{data && …length === 0}` の分岐で読込中に空の表を出していたので、寄せたことで「まだ読めていない」が 0 件から分離された。あわせて 13 表の見た目も変わる（余白がトークン化、罫線が `borderTop`→`borderBottom`、ヘッダが `opacity .6`→`.7`+太字）。`/platform/**` には VRT も axe も無いので視覚の証拠は無い。管理画面側 4 画面の同型欠陥は #966 |
| #899 | 状態表示をバッジへ揃えるかの設計判断（#900 の続き） |
| ~~#966~~ | **2026-09-04 にクローズ**（PR #974 / squash `d98f122`）。管理画面 4 画面（`AssetsManager` / `DepartmentsManager` / `OrganizationsManager` / `StaffManager`）の一覧が「読み込み中」「失敗」を 0 件と断定する。#896 の管理画面側。**一覧の初期値が `useState<T[]>([])` なので「未読」と「0 件」を型で区別できない**ため、`DataTable` へプロパティを配線するだけでは直らず、読み取り状態そのものを足す必要がある。`TableBodyState`（#947）の削除も本 issue で扱う（PR #965 で本番消費者がゼロになった）。独立レビュー 2 周。🔴 **1 周目に「直した」と書いた 2 箇所が直っていなかった** —— `Promise.all` は resolve 後の順序しか直しておらず**接続断で担当者一覧が消えて嘘の原因が出ていた**、`node_modules` の symlink は**先頭コミットに残っていて `git rebase` 中に実体を消した**。オラクル側も 4 種が空虚（既に無効なボタンへの `toBeDisabled`／再マウントする復帰テスト／`allSettled` 化で `catch` が到達不能／1 画面ぶんだけの主張）。**e2e が共有フィクスチャを 2 度壊した**（部署順・担当者の無効化）—— どちらも単独実行では緑で、`--full` で 54 件を巻き添えにして初めて見えた |
| ~~#968~~ | **2026-09-04 にクローズ**（PR #972 / squash `db43964`）。platform の通信失敗が無言になる箇所。issue が名指ししたのは 3 箇所だが、**呼び出し 1 件ごとの検査を入れたら 10 箇所**あった（`FeatureFlags` 3 / `TenantSwitcher` 1 が追加）。最も重い `TenantDetail.runLifecycle` は `try`/`finally` だけで囲われており、テナントの停止/有効化が無言で失敗していた。既存の `platform-list-states.test.ts` が**ファイル単位**だったため原理的に見えなかった型で、オラクルは `tests/config/platform-fetch-failure.test.ts` が粒度を `fetch` 1 件へ落として持つ。**独立レビュー 9 周**（停止条件は 9 周目で達成）。🔴 **reject の先に 3 層あった** —— 非 2xx / 形の壊れた 200 / ハング。特に `{"tenants":[null]}` は `TenantSwitcher` が layout 描画なので `error.tsx` が受けられず、**運用コンソールの全画面**が来訪者向け 4 言語になっていた（`HeaderErrorBoundary` を新設）。教訓は 3 つ: (1)「検査を丸ごと外す」型だけでは**数値/条件を狭める**族が 5 周見えない、(2)**等価だから殺せない**は 2 度とも誤りで、実際は「自分のテストが届いていない」だった、(3) **配線の変異**を 3 度落とした。残りは #973（形の検査が無い platform 7 画面・`ElevationStatus` / `NoticePublishForm` の write timeout）|
| #973 | 管理画面の `fetch` 45 箇所が reject を無言で捨てる（#968 の管理画面側）。**#968 で入れた「呼び出し 1 件ごと」の検出器を `src/components/admin/**` へ広げると出てくる族**。読み取りだけでなく保存・削除・**ログインの送信**も含む（`AdminPasswordLogin` の `POST /api/admin/login` が reject すると押しても何も起きない）。内訳は `DemoStudio` 11 / `ReservationsManager` 5 / `RoutingPolicyManager` 5 ほか。#966 が直すのは 4 ファイルの読み取りだけ |
| ~~#910~~ | **2026-09-03 にクローズ**（PR #953 / #959）。列ソート 7 一覧＋意図的な除外 2 つ、ページング 4 一覧、手書きページ送り 6 つを共有 `ui/Pager` へ寄せ切り。AC2 の「フィルタでページが 1 に戻る」だけは **seed 6 件では 2 ページ目を作れず**実演できないので留保 |
...
このとき意図的に 2 本を残したが、**その判断の根拠は 2 つとも誤りだった**（2026-09-03 夜に検証）。
2 本とも既に削除されており、**どちらも失われていない**:

| 残していたブランチ | 当時の理由 | 検証結果 |
| --- | --- | --- |
| `fix/routing-step-provider-timeout-limit` | PR #944 が CLOSED（未マージ）。「消すと作業そのものが消える」 | 🔴 **誤り。** closed PR でも GitHub は head を保持する。`git fetch origin refs/pull/944/head` で `ce8c2a2` を取得できることを実測した |
| `fix/vrm-observability-wiring` | 「PR が 1 つも無い。素性が確かめられていない」 | 🔴 **記述が stale。** 素性は **2026-08-28 に確定済み**（#830）——「main に無い 116 行」は三点 diff の誤読で、**二点 diff では byte 単位で同一**、内容は **PR #790**（`test(vrm): 観測属性の配線を実描画検査で縛る`・2026-08-24 マージ）で入っていた |

#### 🔴 「ブランチを消すと作業が消える」は closed PR には当てはまらない

**PR を持つブランチは、closed でも `refs/pull/<n>/head` から復元できる。**

```
git fetch origin refs/pull/944/head:refs/tmp/pr944 && git log --oneline refs/tmp/pr944
```

本当に復元経路が無いのは **PR を一度も作らなかったブランチ**だけである（`refs/pull/*` が
生えないため）。掃除の前に確かめるべきは「マージ済みか」ではなく、**「PR があるか」**である:

- **PR がある**（merged / closed どちらでも）→ 消してよい。`refs/pull/<n>/head` で戻せる
- **open な PR がある** → 消さない（PR が壊れる）
- **PR が 1 つも無い** → 消す前に素性を確かめる。ここだけが本当に取り返しがつかない

> **なぜこの訂正が要るか。** 「消すと作業が消える」と書いたまま残すと、次に掃除する人が
> **消してよいものを永久に残す**か、逆に「PR があるから大丈夫」を確かめずに
> **PR の無いブランチを消す**かのどちらかへ倒れる。危険なのは後者だけなので、
> 見分ける条件を書いておく。

> 🔴 **掃除の直後からまた溜まる。そして削除はクラウドからは依然できない。**
> 2026-09-03 夜の実測で、103 本削除の直後に **8 本**まで戻っていた。内訳は
> **マージ済み 7 本**（#959 / #961 / #962 / #964 / #965 / #967 / #969 のぶん）と、
> **PR をまだ持たない 1 本**（`docs/handoff-2026-09-04`）。
> クラウドから `git push origin --delete` を試して `git ls-remote` で確認したところ
> **1 本も消えなかった**（proxy が write を拒否する既知の症状。戻り値は成功に見える）。
> **macOS で流すこと。** 上の「PR があるか」で選別すれば、7 本は消してよく、
> PR の無い 1 本だけ素性を確かめればよい。
>
...
> 🔴 **直近の引き継ぎは `docs/handoff-2026-09-04.md`。** そこの §0 が最重要 ——
> **キューの着手候補 1 位（#815）は自走させてはいけない**（主要 Journey の意味を変える
> 仕様判断なので停止境界に当たる）。自走で回せるのは #960 / #952 で、判断を待たずに
> できる #815 の「計測だけ先に済ませる」案も同節にある。
> VRT の測り方（閾値 0 で測る）と、逃げ道バー・React Compiler の配線の教訓も同書。
> その前は `docs/handoff-2026-09-03.md`（§0 / §4 は消化済み。**§0 の表には誤りがある** ——
> stale の 1 枚目は `kiosk-landscape-target` ではなく `kiosk-landscape-fallback`）。
...
**設定が効いていることも確認済み**（2026-09-08、PR #1015 のマージ直後に
`git ls-remote --heads origin` が `main` だけを返した ＝ `docs/branch-cleanup-resolved` が
**自動で消えた**）。クラウドから叩ける GitHub API（search）は `delete_branch_on_merge` を
返さないので設定値は読めないが、**振る舞いで確かめた**。

🔴 **それでもマージのたびに `git ls-remote --heads origin` を見ること。** 設定は後から
外れうるし、マージ済み PR があるブランチは `npm run evaluate:gate-runs` の
`orphan_branch` に該当しないので、**残っても誰も気づかない**。残っていたらこの表へ
1 行足す（それが唯一の追跡経路である）。
