...
**次に着手する候補（2026-07-30 更新・第 82 wave 消化後）**: **ユーザー操作待ち 1 件**。

> **第 81 wave の記述を訂正**（同一セッション内で自分が stale を作った・分類 stale の 6 回目）。
> 「定期の全体チェック計画が未整備」は**誤り**。#318 はクローズ済みで、`docs/quality-gate.md`
> 「定期運用（#318）」節に方式（週次 Routine / cron 代替）・記録形式・FAIL 時の重大度と期限・
> ツール追従・SKIP=FAIL まで**完全に文書化されている**。着手前に現物を見て判明した。

実際に欠けているのは**実行そのもの**。`docs/gate-runs.md` は EXAMPLE 行だけで実運用の記録が
**0 件**＝定期実行は一度も回っていない。文書が「実際の Routine 作成はユーザーの判断で行う
（自動では作成しない）」と明記しているので、**これはユーザー操作**であってループの作業ではない。

- **ユーザーへの依頼**: 週次 Routine（`./scripts/quality-gate.sh --full --strict` →
  `docs/gate-runs.md` へ追記 → FAIL なら起票）の作成。または cron/launchd での同等運用。
- **ループ側は第 82 wave で対応済み**: 定期実行が文書のみのブランチで走ったときに省略され
  ないよう `--strict` が scope 省略を無効化する（`effectiveScope`）。
- 稼働環境が要る検査（`scripts/url-quality-gate.sh` の ZAP・live lighthouse）は dev 削除済み
  （2026-07-24）のため引き続き回せない。#65 スタック。
