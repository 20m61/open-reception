...
**第 14 wave（次に着手する）**: #361 残(VRT/axe — testing-library/axe の #105 チェック先行)／
#363 残(編集スタジオ UI polish)／ reopenAt のポリシー TZ 整形／ 上記 W3+handler テスト／
**ユーザー判断待ち(deploy のみ)**: #405 Inc2 の Secrets Manager 有効化 deploy・#366 Stack の
deploy(月 $14.2 見積の最終承認)・#4 実資格情報(#65)
