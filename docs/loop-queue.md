...
**次に着手する候補（2026-07-31 更新・第 100 wave 消化後）**

> **#421 の「拠点スコープ」系は一巡した**（第 97〜100 wave）。拠点別 4 画面が URL の
> `siteId` を読み、拠点詳細ハブからそのまま辿れる。残りは下記のとおり。
>
> 0. **#421 の残り（軽い順）**
>    - **受付フローの拠点対応**。`ReceptionFlowsManager` は既定拠点固定のまま。
>      `useSiteScope` + `SiteScopeSelect` を当てるだけで、手順は第 99 wave と同じ。
>      これが済むと `site-destinations.ts` の `siteScoped` を 1 つ真にできる。
>    - **既存画面のテナント解決**。`operating-hours` / `call-routing` 等は
>      `resolveDefaultScope()` でテナントを解決しており、**テナントを切り替えた
>      developer に別テナントの設定を見せうる**（第 100 wave で拠点詳細のみ
>      `resolveActiveTenant` へ是正済み。他画面は未是正）。
>    - **重複ナビ 2 組の一本化** … `kiosks`/`devices`、`call-routes`/`call-routing`。
>      **要ユーザー確認**（概念の一本化＝どちらを残すかの仕様判断）。
