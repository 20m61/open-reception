...
| #966 | 管理画面 4 画面（`AssetsManager` / `DepartmentsManager` / `OrganizationsManager` / `StaffManager`）の一覧が「読み込み中」「失敗」を 0 件と断定する。#896 の管理画面側。**一覧の初期値が `useState<T[]>([])` なので「未読」と「0 件」を型で区別できない**ため、`DataTable` へプロパティを配線するだけでは直らず、読み取り状態そのものを足す必要がある。`TableBodyState`（#947）の削除も本 issue で扱う（PR #965 で本番消費者がゼロになった） |
