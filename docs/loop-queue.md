...
| 順 | Issue | なぜこの順か | 着手可否 |
| --- | --- | --- | --- |
| 1 | **#838** | ゲート任意ツール欠落に SessionStart で気づく。`--full` を回せない環境を e2e 全滅の前に名指しする | ローカル可（実装中） |
| 2 | **#832**（残: Vonage ビデオ / QR） | #323 AC3 の実運用経路。PSTN ポーリングは消化済み。ビデオは設計判断あり | ローカル可（設計判断あり） |
| 3 | **#816** → **#815** | 逃げ道バーが内容を覆う。#816 は linux/darwin 両方の VRT 取り直しが要る | darwin VRT 待ち（#789） |

> **#818 / #817 / #843 / #836 / #849 / #837.2–3 は 2026-09-01 に消化した**
> （PR #852 / #856 / #853 / #854 / #855 / #857。linux VRT 取り直しは #858）。
> #837.1（stall 自己確認）は main の `__stallAt` で既充足。
> #838 AC6（semgrep ルール固定）は #841 で既充足。残りは検知と早期 fail。
