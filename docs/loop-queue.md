...
1. **e2e で platform エリアへ到達できるようにする**（第 85 wave で発見）。platform は developer 専用で、`loginAsAdmin` のパスワードログインは developer にならない。そのため `/platform/*` の e2e は 1 本も実効していない（`capture-screens.spec.ts` は撮るだけなので気づけなかった）。`OPEN_RECEPTION_ADMIN_PASSWORD_ROLE=developer` を e2e の webServer env に足すと admin 側の挙動も変わるため、**developer 専用セッションを別に張る helper** を作るのが筋。これが入ると `platform-viewing-context.spec.ts` の 4 本が自動で走り出す。
