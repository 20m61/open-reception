...
**次に着手する候補（2026-07-30 更新・第 81 wave 消化後）**: **定期の全体チェック計画**。
第 81 wave で文書のみの周回のゲートを 152s へ短縮したが、**省略した分を定期実行で必ず一度は
踏む網が無い**。現状の担保はトリップワイヤ（typecheck/lint/unit/secrets は必ず回る）だけ。
設計すべきもの: `--full --strict` の頻度と起点 / `--full` に入っていない検査の位置づけ
（`scripts/url-quality-gate.sh` の ZAP と live lighthouse は**稼働環境が要る**ので今は回せない。
dev は 2026-07-24 削除済み）/ FAIL 時の起票と扱い / `docs/gate-runs.md` への記録運用。
土台は本書と `docs/quality-gate.md`「定期運用（#318）」節。
