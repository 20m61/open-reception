...
| #968 | platform の通信失敗が無言になる箇所。**2026-09-03 に消化**。issue が名指ししたのは 3 箇所だが、**呼び出し 1 件ごとの検査を入れたら 10 箇所**あった（`FeatureFlags` 3 / `TenantSwitcher` 1 が追加）。最も重い `TenantDetail.runLifecycle` は `try`/`finally` だけで囲われており、テナントの停止/有効化が無言で失敗していた。既存の `platform-list-states.test.ts` が**ファイル単位**だったため原理的に見えなかった型で、オラクルは `tests/config/platform-fetch-failure.test.ts` が粒度を `fetch` 1 件へ落として持つ |
