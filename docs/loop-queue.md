...
**次に着手する候補（2026-08-01 更新・第 110 wave 消化後）**

> **最優先は #554 の残り 5 画面**（signage / stay / reservations / staff-response / demo）。
> 第 109 wave で 1 画面目（experience-versions）の型ができ、第 110 wave で共有基盤
> （締切・再試行・取得状態の消費）が入ったので、**移行した画面は復帰経路を自動で継承する**。
> 写すときは `resolveVersionActions` と同じく**ハンドラとボタンが同じ 1 つの値を見る**形にし、
> `SiteScopeSelect` の必須 prop `onRetry` を必ず配線すること（型が落とすので忘れようがない）。
> なお `/admin/demo` は `siteId={String(tenantId)}` ＝**拠点 ID にテナント ID を渡している**ので、
> 移行前に別途調査が要る。次いで #557（浅い clone でのゲート base 誤解決）。
