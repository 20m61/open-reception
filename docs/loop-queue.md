...
> 🔴 **未完の引き継ぎがあるなら、キューより先にそれを読む。** 直近: **`docs/handoff-2026-08-26.md`**。
>
> **`/loop-round` は Issue の 1 周を回すスキルで、引き継ぎを読まない。** 前セッションが
> 「PR のマージ待ち」「ゲートの回し直し」「stash の pop」で終わっている場合、それらは
> **Issue の周回ではない**ので `/loop-round` からは拾われず、放置される。ここに置くのは、
> キューを見る導線（`/loop-round` / `/issue-ac-mapping`）が必ず本書を通るため。
>
> 引き継ぎを消化し終えたらこの注記を消すこと（残すと次の人が古い引き継ぎを追う）。
