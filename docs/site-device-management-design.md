...
### #274 ②/#284: 永続化の repository 標準化と死活集計の境界クエリ恒久化（実装済み）

- **repository 標準化（#274 ②）**: device/kiosk の永続化を §9 標準
  （docs/persistence-design.md）へ統合した。
  - kiosk-store は `KioskRepository`（`src/lib/kiosk/repository.ts`、getBackend() 委譲の
    単一実装）+ kiosk-store のプロセス共有ファクトリ/互換 API に再構成（呼び出し側は無変更）。
  - tenant の `memory-repository.ts`（MemoryTenantStore）は廃止。サービステストは
    memory backend + seed で `DataBackedTenantStore` を直接検証する（二重実装の解消）。
- **境界クエリの恒久化（#284）**: 死活集計（summarizeFleet 経路）の無境界
  `listAllDevices()` を廃止し、**「テナント一覧起点 + テナント毎の境界クエリ
  `listDevicesByTenant`」** で集約する方式（選択肢 a）を採用した。
  - 実装: device collection に `indexedField: 'tenantId'` を指定し、
    `Collection.listByIndex` → dynamo は **既存 GSI1 の再利用**
    （GSI1PK=`col#device#idx#<tenantId>`。CDK 追加・実デプロイ不要）、memory は等価フィルタ。
    `listDevices(tenantId, siteId)` も同クエリ + siteId フィルタになり、管理 API の読みも
    テナント境界に比例する。
  - 選定理由: (b) lastSeenAt GSI は online 数しか境界化できず（offline/total/maintenance/
    disabled と kiosk union の id 突合に全端末レコードが必要）、(c) 維持カウンタは online が
    時刻窓依存のため不向き + 全 write 経路のカウンタ整合コストが高い。(a) はテナント数が
    契約規模で小さく、テナントスコープの死活集計（#284 残増分）にそのまま繋がる。
  - TTL キャッシュ（30 秒、device-fleet.ts）は併用のまま（読み頻度の抑制はキャッシュ、
    読み量の境界化はクエリ、と役割分担）。
  - **既存データの backfill**: GSI1 キーは put 時に書くため、既存 dynamodb の device
    アイテムは再 put まで境界クエリに現れない（デプロイ増分で seed 再実行 or 再保存。
    kiosk レジストリ union が表示のセーフティネットになる）。memory backend は影響なし。
...
- ~~Kiosk の enabled/失効を Device へ写像する逆方向の同期~~（#261 で heartbeat 起点、#284 inc1 で
  ...
  デバイス id 化を含む）。
- ~~端末台数スケール時の死活集計の境界クエリ化~~（#274 ②/#284 で実装済み: tenantId 境界クエリ
  + テナント一覧起点の集約。上記「#274 ②/#284」参照。残りは既存 dynamodb データの backfill を
  デプロイ増分で行うことと、実 actor 解決後のテナントスコープ死活集計）。
