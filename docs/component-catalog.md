...
### 5.1 `*Manager` のインライン UI → `ui/` 移行（increment 3）

increment 3 で `/admin` 配下の各 `*Manager.tsx` に散在するインライン UI（素の `<button>` /
`<table>` / 入力 + ラベル / カード）を共有 `ui/` プリミティブへ寄せた。**公開 props・主要
data-testid・挙動は不変**（リファクタ）。消費側のページ / API / 認可は無改変。

DataTable に移行元の行/セル属性を保つための上書き口を追加した（ui/ 拡張）:
`rowTestId(row,i)`（行 `<tr>` の data-testid）、`rowProps(row,i)`（draggable/onDrop 等の行
レベル属性）、Column の `cellTestId(row)` / `cellStyle(row)`（セル `<td>` の testid・色）。

| 既存 `*Manager`                | 寄せ先（`ui/`）                                  | 状態 / 移行メモ |
| ------------------------------ | ----------------------------------------------- | -------------- |
| `SitesManager`                 | `Button` / `Field` / `DataTable`(+`EmptyState`)  | ✅ 追加フォーム→`Field`+`Button`、一覧→`DataTable`。`site-*` testid（`site-table`/`site-row`/`site-name`/`site-devices`/`site-edit-input`/`site-save`/`site-edit`/`site-toggle`/`site-name-input`/`site-add`）維持 |
| `AssetsManager`                | `Button` / `Field` / `FormRow` / `DataTable`     | ✅ `asset-*` testid（`asset-table`/`asset-row`/`asset-name-cell`/`asset-active`/`asset-activate`/`asset-toggle`/`asset-kind`/`asset-name`/`asset-url`/`asset-add`/`asset-error`）維持 |
| `MotionsManager`               | `Field` / `DataTable`                            | ✅ default 選択→`Field`、状態×モーション表→`DataTable`。`motion-table`/`motion-row`/`motion-default`/`motion-<key>` の aria-label・testid 維持 |
| `VoiceManager`                 | `Button` / `Field` / `FormRow`                   | ✅ 入力行→`Field`/`FormRow`、保存→`Button`。チェックボックスは inline 維持。`voice-*` testid 維持 |
| `DepartmentsManager`           | `Button` / `Field` / `DataTable`                 | ✅ DnD は `DataTable` の `rowProps`(draggable/onDrop)で再現。`dept-table`/`dept-row`/`dept-name`/`dept-toggle`/`dept-name-input`/`dept-kana-input`/`dept-add` 維持。`aria-label="up"/"down"` のソートボタンも維持。CsvImport は無改変 |
| `StaffManager`                 | `Button` / `Field` / `DataTable`                 | ✅ 一覧→`DataTable`。`staff-availability` はセル testid、行内 `StaffEditor` は actions セル内に保持。`staff-*` testid 維持 |
| `SecurityManager`              | `Button` / `Field`                               | ✅ 緊急停止/保存/再開ボタン→`Button`(`danger`/`primary`)、PIN/IP 入力→`Field`。`emergency-section` は状態で warning/danger を切替えるため器は据置。`emergency-*`/`security-*` testid 維持 |
| `CallRoutesManager`            | `Button` / `Field` / `Card`                      | ✅ ルートカード→`Card`(testId=`route-card`)、ボタン→`Button`(削除=`danger`)。`route-*` testid 維持。RouteFlow 可視化は無改変 |
| `LanguageSettingsManager`      | `Button` / `Field`                               | ✅ 初期表示言語 select→`Field`、保存→`Button`。有効言語の fieldset/legend は a11y のため据置。`lang-*` testid 維持 |
| `ReceptionFlowsManager`        | `Button` / `Field` / `Card`                      | ✅ フローカード→`Card`(testId=`flow-card`)、ボタン→`Button`(削除=`danger`)。`flow-*` testid 維持。FlowSummary 可視化は無改変 |
