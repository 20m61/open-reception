...
- **inc3（#83 のスコープ分割）**: 残りの read 配線を小さく安全な増分へ分割し、書き込み（破壊的操作）
  ...
  - **inc3e（実装済）**: 障害（Incident）＋予定メンテナンス（MaintenanceWindow）＋お知らせ（Notice）の
    状態 read を `/platform/maintenance` へ追加（`summarizeIncidents`/`summarizeMaintenanceWindows`/
    `summarizeNotices`＋各 store。seed は memory 専用＝本番 DynamoDB はダミーを出さない）。いずれも
    対象テナント選択（inc3b-2）で全体影響＋選択テナントに絞り込む。発動/登録は影響範囲表示 + 昇格 +
    監査（inc4）。
- **inc4 以降（書き込み・安全装置）**:
