...
- 🔴 **読み取りコマンドの引数として現れただけではブロックしない (#960)。**
  ...
  判定は**パイプライン単位・既定 deny**で、落とすのは全段が読み取り専用コマンド
  （`grep` / `rg` / `cat` / `head` / `sed` / `wc` / `ls`、および読み取り系サブコマンドの
  `git`）のときだけ。実行しうる語（`npx` / `node` / `xargs` / `bash` / 未知のコマンド）が
  1 段でも混じれば従来どおり見る。
  🔴 **allowlist には「他プロセスを起動しない道具」しか入れない。** `find` / `fd` /
  `sort`（`--compress-program`）/ `awk`（`system()`）は起動できるので入れていない ――
  道具を 1 つ足すたびに同型の穴が増える族なので、疑わしいものは deny 側に置く。
  🔴 **「段の連結」と「実行の境界」を区別する。** `|` / `|&` / サブシェル・コマンド置換・
  プロセス置換の括弧 / backtick は**同じ塊の段**として残す（`;` `&&` `||` `&` 改行だけが境界）。
  ここを取り違えると、読み取りの段だけが落ちて実行の段に検出語が残らない ――
  `cat scripts/merge-pull-request.ts | bash` と同じ穴が `|&` や `bash <(cat …)` という
  別の綴りで開く。標準出力のリダイレクト（`>` / `>>`）も「読み取りの結果を置いてから実行する」
  綴りなので read 扱いにしない（`2>` は対象外 ―― `grep … 2>/dev/null` は日常的に打つ）。
  🔴 **代入だけの塊を read にしない。** `SCRIPT=scripts/merge-pull-request.ts; npx tsx $SCRIPT`
  で検出語が消える。
  同時に、**空白を含まない引用は中身を残す**ようにした。変更前は
  `npx tsx "scripts/merge-pull-request.ts" 1` が素通りしており（実測）、迂回が
  「クォートを 2 つ足す」で済んでいた。空白を含む引用（＝散文・コミットメッセージ）は
  従来どおり落とす。**引用の中のメタ文字は区切りとして再解釈させない**（潰してから割る）――
  さもないと `rg -n 'create|merge' <path>` が `|` で割れて誤ブロックになる。
  脱出ハッチの代入は**塊の先頭**でのみ認める（`echo "VAR=1" && gh pr merge` を迂回にしない）。
  🔴 **判定に使う道具（perl / awk）が落ちたら deny 側へ倒す。** 空の判定結果を返すと
  ガードが丸ごと無言で無効化される（fail-open）。判定不能なら生のコマンドをそのまま
  素朴に包含判定する。
  回帰行列は `tests/hooks/pr-gate-guard.test.ts` の `EXECUTION_FORMS`（44 形）/
  `READ_INTO_EXECUTOR` / `READ_FORMS`（26 形）で、**散文ではなくこの配列が正本**。
  同じ 70 形を 3 版へ当てた実測:

  | 版 | 実行形 44（ブロックすべき） | 読み取り形 26（通すべき） |
  | --- | --- | --- |
  | 変更前（`b9f9718`） | 42 / 44（引用形 2 が素通り） | 6 / 26 |
  | 中間版（`c1f81ea`・レビュー前） | **32 / 44** | 21 / 26 |
  | 現在 | **44 / 44** | **26 / 26** |

  🔴 **中間版は実行形について変更前より弱かった。** 「誤発火を消す」変更が、読み取り扱いの
  抜け道（代入で運ぶ / `|&` / プロセス置換 / リダイレクト / `fd -x` 等）を 9 族ぶん開けていた
  ―― 自分で当てた 18 形は全部 kill しており、**族ごとの見落としは独立レビューでしか出なかった**。
  実装への変異 20 種は行列と静的検査で全 kill（allowlist の中身とフラグ族は振る舞いでは
  殺せないので、静的検査で縛っている）。
