...
- 🔴 **読み取りコマンドの引数として現れただけではブロックしない (#960)。**
  ...
  1. **シェルの機能を一切使わない読み取りパイプラインなら、そこで通す。**
     `;` `&` `|`（パイプは可）`<` `>`（`2>/dev/null` と `2>&1` を除く）`(` `)` backtick
     `$` **改行**を引用の外に含まず、引用が閉じていて、各段が
     「無害な環境変数（`LC_*` / `LANG` / `TZ`）＋ 読み取り専用の道具（`git` は読み取り
     サブコマンドのみ）」で、**検出語が位置引数として現れている**こと
  2. 外れたら**従来どおり**コマンド文字列そのものへパターンを当てる

  🔴 **この向きが肝である。** 最初の 2 版は逆向き（読み取りに見える塊を判定対象から落とす）
  で、独立レビューが 2 周にわたって抜け道を出し、**そのたびに綴りを 1 つ足す**形になった
  （`|&` → プロセス置換 → `&>` → `sed --in-place` → `git grep -O` …）。
  `.claude/rules/opus5-autonomous-loop.md`「値を調整している自分には気づけない」の型である。
  whitelist なら**見落とした綴りは常にブロック側へ倒れる**ので、綴りを足し忘れても穴にならない。
  ...
  | 通っていた形 | 何が起きるか |
  | --- | --- |
  | `GIT_EXTERNAL_DIFF=<検出語> git diff` | **実際にスクリプトが起動する**（先頭の環境変数代入を読み飛ばしていた） |
  | `PATH=./bin:$PATH cat <検出語>` | `./bin/cat` が動く（先頭語の綴りだけを見る判定の前提が壊れる） |
  | `sed -n "w /tmp/m.ts" <検出語>` | 読んだ内容を別の場所へ置ける（GNU sed は `e` で起動もできる） |
  | `./bin/cat <検出語>` | basename へ正規化していたので綴りを詐称できた |

  よって: **環境変数代入は無害な名前だけ**（値の allowlist はテストが静的に縛る）／
  **先頭語は正規化しない**（フルパス起動は allowlist に無い綴りとして落ちる）／
  **`sed` は allowlist に入れない**（`w` / `e` / `r` を素朴に判定できない）。

  🔴 **引用はシェルの規則どおりに走査する。正規表現で対消ししない。**
  `cat "'" a.txt; gh pr merge 12 "'"` は、正規表現だと引用が「二重引用符の中の単引用符
  2 つ」で対になり、**その間の `;` ごと消えて** `gh pr merge` が判定から落ちる
  （**変更前からあった穴**。3 周目のレビューが実証）。1 段目・2 段目とも同じ走査を使い、
  2 段目では**空白を含む引用から来たトークンだけ**を散文として捨てる
  （ただし `$(…)` と backtick は引用の中でも展開されるので捨てない）。
  空白を含まない引用は中身を残す ―― 変更前は
  `npx tsx "scripts/merge-pull-request.ts" 1` が素通りしており、迂回が「クォートを
  2 つ足す」で済んでいた。

  🔴 **判定に使う道具（`perl` / `jq`）が落ちたら deny 側へ倒す。** 空の判定結果を返すと
  ガードが丸ごと無言で無効化される（fail-open）。`jq` が落ちたときは payload を
  読めていないので、引用の除去をやめて記号を空白へ潰して検査し、**MCP のツール名も見る**
  （2026-08-21 に main を red にした経路がそこだけ開いていた）。この潰しは bash の置換で
  行う ―― 外部コマンドを挟むと「道具が落ちている」経路に穴がもう 1 段増える（実測）。

  回帰行列は `tests/hooks/pr-gate-guard.test.ts` の `EXECUTION_FORMS`（84 形）/
  `READ_FORMS`（43 形）で、**散文ではなくこの配列が正本**。同じ 127 形を 5 版へ当てた実測:

  | 版 | 実行しうる形 84 | 読み取り形 43 |
  | --- | --- | --- |
  | 変更前（`b9f9718`） | 79 / 84 | 6 / 43 |
  | blacklist 版（`6bba78b`） | **52 / 84** | 38 / 43 |
  | whitelist 版（`1616e8f`） | **65 / 84** | 34 / 43 |
  | 走査版（`04ebbc1`） | **76 / 84** | 42 / 43 |
  | 現在 | **84 / 84** | **43 / 43** |

  🔴 **走査版で落としたのは「改行」と「語中の `#`」だった。** 改行を `/\s/` に食わせると
  `git status --short<改行>gh pr merge 997` が 1 段目を通り、`#` を語中でもコメント扱いに
  すると `cat a.txt#z; gh pr merge 12` の `;` 以降が消える。**どちらも前の版が守っていた
  保証**で、方式を替えたときに落ちた（規約「方式を替えたら、前の方式が守っていた変異を
  当て直す」の実例。4 周連続で同じ型が出ている）。

  意図的に**通さない**読み取り（誤ブロックとして受け入れたもの。いずれも変更前と同じ挙動）:
  `sed -n 1,40p <path>` / `/usr/bin/grep …`（フルパス起動）/ `… < /dev/null` /
  ...
  実装への変異 21 種は行列と静的検査で全 kill（allowlist の中身は振る舞いでは殺せないので、
  **コメントを除去したうえで**静的に縛る）。**各周とも「自分で当てた変異は全部 kill」と
