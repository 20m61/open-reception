...
- 🔴 **読み取りコマンドの引数として現れただけではブロックしない (#960)。**
  ...
  1. **シェルの機能を一切使わない読み取りパイプラインなら、そこで通す。**
     `;` `&` `|`（パイプは可）`<` `>`（`2>/dev/null` を除く）`(` `)` backtick `$` を
     引用の外に含まず、引用が閉じていて、各段が
  ...
  **`sed` は allowlist に入れない**（`w` / `e` / `r` を素朴に判定できない）。
  ...
  回帰行列は `tests/hooks/pr-gate-guard.test.ts` の `EXECUTION_FORMS`（73 形）/
  `READ_FORMS`（42 形）で、**散文ではなくこの配列が正本**。同じ 115 形を 4 版へ当てた実測:

  | 版 | 実行しうる形 73 | 読み取り形 42 |
  | --- | --- | --- |
  | 変更前（`b9f9718`） | 68 / 73 | 6 / 42 |
  | blacklist 版（`6bba78b`） | **46 / 73** | 37 / 42 |
  | whitelist 版（`1616e8f`） | **59 / 73** | 34 / 42 |
  | 走査版（現在） | **73 / 73** | **42 / 42** |
