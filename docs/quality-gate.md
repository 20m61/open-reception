...
```
▶ 一時領域 (#721/#1136)
  /tmp の空き: 22.5G（開始時: 26G）/ エントリ: 6200 件
  内訳:    3200 aws-preflight-test-#-    1800 aws-diff-gate-test-#-     900 gate-stamp-
  残骸: cdk.out 0 件 / 周回 root 0 件
  ⚠ /tmp のエントリが 6200 件あります（しきい値 5000）。**容量が空いていても**
    unit が `Test timed out` で落ちる原因になります（#1136。単独実行で通るなら偽の赤）
```

（上は #1136 が起きた分布を合成して**実際に走らせた**出力。空きは 22.5G あるのに
「どの族が積もっているか」がそのまま読める、というのがこの節の狙いである）
...
🔴 **容量だけを見ていると見えない族がある (#1136)。** 2026-09-17、`/tmp` に
**18,726 エントリ**（うち 10,317 件が 1 つのテストファイル由来）が積もり、
`tests/config/capability-doc-sync.test.ts` が `Test timed out in 5000ms`（実測 8751ms /
単独実行では 663ms）で落ちた。**ディスクは 22G 空いていたのでこの節は「正常」と表示した。**
そこで **エントリ件数**と**族の内訳**も必ず出す:

- `エントリ: N 件` … `<tmp>` 直下の総数。数え上げではないので新しい prefix にも追随する
- `内訳: …` … 上位 3 族（件数つき）。**しきい値に関係なく常に出す** ——
  「多いのかどうか」が判断できないと意味が無いため
- しきい値は 5,000 件（`TEMP_ENTRY_WARN_THRESHOLD` で上書きできる）。超えたら ⚠ を付ける

族の括り方は **拡張子 → 末尾トークン → 数字列**の順に落とす
（`sed -E 's/\.[A-Za-z0-9]{1,6}$//; s/([-._])[A-Za-z0-9]*$/\1/; s/[0-9]+/#/g'`）。
🔴 **ここは 2 度作り替えており、1 回目の替えで退行させた**ので経緯を残す:

| 方式 | `<prefix>-<pid>-<rand>.json`（#1136 の当事者） | `<prefix>-<rand6>` | 区切り無し（`vitest`） |
| --- | --- | --- | --- |
| 最初の数字で切る | ✅ 1000 件として出る | ❌ 砕ける（実 `/tmp` で被覆 6%） | ✅ |
| 最後の `-`/`.` まで | ❌ **1 件と出る**（拡張子だけ落ちる） | ✅ | ✅ |
| 現行（3 段） | ✅ `1000 aws-preflight-test-#-` | ✅ | ✅ |

**「実 `/tmp` で測ったら改善した」だけで替えたのが誤りだった** —— 移行後の `/tmp` には
素のファイル漏洩がもう無いので、**置き換える側が守っていた分布が標本に入っていなかった**。
両方の分布を `tests/config/gate-temp-report.test.ts` の回帰に入れてある。

### unit の一時領域は**テストファイルごとに隔離**する (#1136)

`vitest.config.ts` の `setupFiles`（`tests/setup/temp-isolation.ts`）が、テストファイル
ごとに `<tmp>/or-vitest/XXXXXX` を切って `process.env.TMPDIR` をそこへ
向ける。`os.tmpdir()` は `TMPDIR` を**呼び出しのたびに読む**ので、**どの綴りで作られた
一時パスも**その root の中に落ち、ファイル終了時に root ごと消える。

実測: `/tmp` 直下に増えるのは unit レーン全体で **1 エントリ**（`or-vitest`）。
`--full` 1 本を回して、テストヘルパの prefix で作られた `/tmp` 直下のエントリは **0 件**。

🔴 **ソース走査で「作り方の綴り」を数え上げる方式は撤回した。** 呼び出しの綴り →
import の綴りと 2 度作り替えたが、独立レビューの実測で **16 綴りのうち 10 が素通り**した
（名前空間 import・`node:` 無しの `from 'os'`・prettier が普通に生成する**複数行 import**・
`require`・動的 `import()`・re-export・`'/tmp/...'` 直書き）。#1136 の支配的な原因を
復活させても**ガードは緑だった**。綴りを足す対処はこのリポジトリが何度も撤回した型なので、
**方式を裏返した**（`.claude/rules/opus5-autonomous-loop.md`「数え上げない」）。

走査（`tests/config/temp-cleanup-guard.test.ts`）が見張る面は**隔離を迂回する 2 つだけ**に縮んだ:

1. `/tmp` / `/var/folders` を**絶対パスで直書き**する（`TMPDIR` を通らない）
2. テストが自分の `process.env.TMPDIR` を**上書き**する（隔離を自分で外す）

正当な逸脱（実在しない偽パスを渡す negative case 等）は行末の `// temp-ok: <理由>` で抜く。

テストごとに小さく保つ作法としては `tests/helpers/temp.ts`（`makeTempDir` /
`makeSharedTempDir` / `makeTempFile`）を使う。**リークの防壁は隔離側**なので、
helper は「作法」であって機構ではない。

🔴 **パスを深くしすぎない。** 隔離は一時パスに 17 文字（`or-vitest/XXXXXX`）足すので、
`tsx` のような **UNIX ドメインソケットを掘る道具**の余白（`sun_path` は 108 バイト）を食う。
実測（TMPDIR が 96 文字の環境でフル unit）: **main でも 29 テストが `EADDRINUSE` で落ちる**
／名前が 30 文字だと **74 テスト**まで悪化／17 文字なら **8,889 passed で全緑**
（ファイルごとに `tsx-0` が分かれるぶん、隔離は既存の衝突をむしろ減らす）。
症状はアサーション到達前の失敗なので、**偽の赤として読み間違えやすい**。

🔴 **run 全体が kill されると `afterAll` が走らず root が残る**（`finally` でも防げない）。
**全テストが skip されたファイル**と**収集時にエラーになったファイル**でも `afterAll` は
走らないので、そのファイルごとに空の root が 1 件残る（TDD の red 中は後者が日常的に起きる）。
`process.on('exit')` では塞げない —— vitest のワーカーでは発火しない（実測）。

🔴 **走行中の `<tmp>/or-vitest` を手で消さないこと。** 件数が増えると消したくなるが、
並列トラックの走行中に消すと**生きている root** が消え、症状は
「アサーション到達前のハング」になる（実測で 54s → 900s の timeout）。
掃除は 6 時間より古いものを隔離側が自動で行う。手で消すならゲートの外で。

🔴 **`<tmp>/or-vitest` は固定名なので、`/tmp` を共有するマルチユーザ環境では
別ユーザが先に作ると `EACCES` で全テストが死ぬ**（クラウドは単独ユーザ、macOS は
per-user の `TMPDIR` なので今日は踏まない）。踏んだら root 名に uid を混ぜる。
残骸は 1 段下に隠れるので、上の 1 行が **`unit 隔離 root N 件`** として**潜って数える** ——
直下の総数からは 1 件にしか見えないため。隔離の setup は起動時に **6 時間より古い** root を
掃く（走行中の並列トラックを壊さないよう age で判断する。#721 の infra と同じ扱い）。
...
- 🔴 **生の REST での PR 作成（`POST .../pulls`）は見ていない（既知の穴 / #1120）。**
  #1117 で一度塞ごうとして**撤回した** —— URL と「本体を送る形」の AND で判定したところ、
  止めたい形を 5 通り以上取りこぼし（`gh api …/pulls -f title=x` ＝ `gh api` での PR 作成の
  最も普通の形を含む）、止めてはいけない読み取りを 3 通り止めた。**誤検出はガードごと
  迂回される動機を作る**（#960）ので、薄い保護と引き換えにしない。塞ぐなら数え上げでは
  なく「読み取りと判る option の allowlist へ裏返す」必要があり、#1120 で扱う。
- 🔴 **読み取りコマンドの引数として現れただけではブロックしない (#960)。**
  ...
  何もマージしないので、この発火はガードの目的に寄与しない一方、誤発火が続けば
  `OPEN_RECEPTION_SKIP_GATE_GUARD=1` の常用 ―― **override の習慣化** ―― へ倒れる。

  判定は 2 段で、**1 段目は whitelist**（実行形の blacklist ではない）:

  1. **「シェルの機能を一切使わない読み取りパイプライン」なら、そこで通す。**
  2. 外れたら**従来どおり**コマンド文字列そのものへパターンを当てる

  🔴 **1 段目の設計原則は「解釈しない」である。**
  バックスラッシュ・`$`・backtick・波括弧・改行が現れたら、**その時点で 2 段目へ落とす**。
  引用の外の `;` `&` `<` `>` `(` `)` も同じ。残った部分集合では、語の切れ目は
  「引用の外の空白」だけで決まり、**bash と一致する**（エスケープも展開も無いため）。

  この原則に辿り着くまで独立レビューを 5 周し、**毎周「変更前が止めていた実行形が通る」**
  という同じ型の BLOCKER が出た。原因はすべて「シェルを解釈しようとして bash とずれた」ことで、
  綴りを 1 つずつ足しても終わらなかった:

  | 周 | 通っていた形 |
  | --- | --- |
  | 1 | `|&` / プロセス置換 / コマンド置換 / 代入で運ぶ / リダイレクト / `fd -x` / `sort --compress-program` / `git -c core.pager` / `sed -i` |
  | 2 | `&>` 系 / `sed --in-place` / `git grep --open-files-in-pager` |
  | 3 | `GIT_EXTERNAL_DIFF=<検出語> git diff`（**実際に起動した**）/ 引用の対消しで `;` が消える / `sed` の `w` / `./bin/cat` |
  | 4 | 改行が「ただの空白」/ 語中の `#` / `git show --output <file>` / 劣化した `jq` |
  | 5 | `uniq <検出語> <出力先>`（第 2 位置引数が出力）/ 行継続で語をつなぐ / `"$(gh pr merge)"` / `gh pr >/dev/null merge` |

  **`.claude/rules/opus5-autonomous-loop.md`「値を調整している自分には気づけない」の実例。**
  解釈をやめたことで、この族は構造的に発生しなくなった（解釈しなければ差は生まれない）。

  1 段目が通すのは、上に加えて次を満たすものだけ:

  - 各段が「無害な環境変数（`LC_*` / `LANG` / `TZ`）＋ 読み取り専用の道具」
    （`git` は読み取りサブコマンドのみ。先頭語は**正規化しない**ので `./bin/cat` は落ちる）
  - **検出語が位置引数**として現れている（`--pager=<検出語>` のように option の値に置くと、
    読み取りの道具がそれを**起動する**）
  - allowlist には他プロセスを起動しない／ファイルを書けない道具しか入れない
    （`find` / `fd` / `sort` / `awk` / `sed` / `less` / `more` / `xargs` / `tee`、
    そして **`uniq`** ―― 第 2 位置引数が出力ファイルになる）
  - 🔴 **1 段目は「作業証明」を見る。** 終了コードだけで判断すると、`perl` が
    「落ちずに何もせず成功」しただけで判定が丸ごと許可へ倒れる

  🔴 **判定に使う道具（`perl` / `jq`）が落ちても・空を返しても deny 側へ倒す。**
  `jq` が読めないときは payload を素朴に検査し、**MCP のツール名も見る**
  （2026-08-21 に main を red にした経路がそこだけ開いていた）。記号の潰しは bash の置換で
  行う ―― 外部コマンドを挟むと「道具が落ちている」経路に穴がもう 1 段増える。

  2 段目は 1 段目と同じ走査を使い、**空白を含む引用から来たトークンだけ**を散文として捨てる
  （`$(…)` と backtick は引用の中でも展開されるので捨てない。裸の `$VAR` は捨てる ――
  さもないと `git commit -m "… ($USER)"` が誤ブロックになる）。行継続は先に畳み、
  リダイレクトの矢印と行き先は捨てる（`gh pr >/dev/null merge 12` で語が分断されるため）。

  回帰行列は `tests/hooks/pr-gate-guard.test.ts` の `EXECUTION_FORMS`（116 形）/
  `READ_FORMS`（43 形）/ `DELIBERATELY_BLOCKED`（7 形）/ `KNOWN_PRE_EXISTING_GAPS`（4 形）で、
  **散文ではなくこの配列が正本**。同じ 159 形を 8 版へ当てた実測:

  | 版 | 実行しうる形 116 | 読み取り形 43 |
  | --- | --- | --- |
  | 変更前（`b9f9718`） | 101 / 116 | 9 / 43 |
  | blacklist 版（`6bba78b`） | **65 / 116** | 40 / 43 |
  | whitelist 版（`1616e8f`） | **80 / 116** | 33 / 43 |
  | 走査版（`04ebbc1`） | **91 / 116** | 39 / 43 |
  | 走査版 2（`84dd9c0`） | **99 / 116** | 40 / 43 |
  | 非解釈版（`f78735f`） | **103 / 116** | 40 / 43 |
  | 機構を外した版（`b2848b0`） | **112 / 116** | 41 / 43 |
  | 現在（option も allowlist） | **116 / 116** | **43 / 43** |

  🔴 **7 周のレビューが到達した原則は 3 つある。**

  1. **解釈しない** … シェルを解釈しようとするたび bash とずれた（`|&` → プロセス置換 →
     `&>` → 環境変数 → 改行 → 行継続 → 語中の `#`）。現在は
     バックスラッシュ・`$`・backtick・波括弧・改行が現れた時点で 2 段目へ落とす
  2. **数え上げない** … 「起動器を渡せる option 名」を列挙したところ、`rg --hostname-bin` が
     漏れて**実際にプログラムが起動した**（7 周目）。道具も option も allowlist にし、
     **知らないものは通さない**
  3. **機構を足さない** … `gh pr >/dev/null merge 12`（変更前から通っていた）を塞ごうと
     リダイレクト処理を足したところ、それが `;` と次のコマンド名まで飲み込み、
     `npm run build >/tmp/b.log 2>&1;gh pr merge 997` という**日常的な形**を素通しにした
     （6 周目）。塞ぎにいった穴より、足した機構のほうが危なかった。元の穴は
     `KNOWN_PRE_EXISTING_GAPS` として固定し **#998** で追跡する

  意図的に**通さない**読み取り（`DELIBERATELY_BLOCKED`。誤発火として受け入れたもので、
  いずれも変更前と同じ挙動）: バックスラッシュを含む形 / `uniq` / `sed -n 1,40p <path>` /
  フルパス起動 / 複数行 / プロセス置換。**穴を開けるより誤発火を残すほうが安い。**

  🔴 **このガードが止めているのは「うっかり」であって、故意の迂回ではない。**
  2 コマンドに分ければ何でもできるし、`bash -c "..."` はどちらの段にも掛からない
  （**変更前からそう**）。止めたいのは「ゲートを回し忘れたまま PR / マージへ進む」ことだけで、
  意図的な迂回には transcript に残る脱出ハッチがある。

  実装への変異 28 種は行列と静的検査で全 kill。**各周とも「自分で当てた変異は全部 kill」と
  報告しており、族ごとの見落としは独立レビューでしか出ていない**（9 → 4 → 3 → 4 → 3 族）。
  ...
  **PR を作らずに終わり**、FAIL が 5 日間 main に載らなかった（#656）。当時この手順は
  ...
  - 🔴 **GitHub CLI (`gh`) は使わない (#1117)。** 経緯は 2 段ある:
    ...
    **`GH_TOKEN` / `GITHUB_TOKEN` の有無で publish の可否を判定しない** —— 在る環境では送り、
    ...
    ブランチ数によっては 403 を踏む。
  - 🔴 **到達性は 2 箇所で確かめる (#1117)。** `--publish` は
    `scripts/check-publish-path.ts`（`GET /repos/{owner}/{repo}` が 2xx か、だけを見る）を
    **ゲートの前**と **`git push` の直前**の 2 回通す。役割が違う:

    | 呼ばれる場所 | 役割 |
    | --- | --- |
    | ゲートの**前** | **報告だけ。絶対に止めない** |
    | `git push` の**直前** | 到達できなければ **push せずに**非 0 で終える（exit 5） |

    🔴 **ゲートの前で止めない。** 記録の追記も `evaluate:gate-runs` も `loop:retro` も
    publish の**後ろ**に居るので、止めると FAIL の測定そのものが消える ―― #656 より悪い。
    加えて **node_modules がまだ無い fresh clone**（＝クラウド週次 routine の既定の姿）では
    `npx --no-install tsx` 自体が失敗するので、ここで止める設計は「毎週 1 秒も走らない」に
    倒れうる（実測。独立レビューが BLOCKER として指摘した）。

    🔴 **push の直前で止める。** #656 の被害は「push されたブランチに PR が無いまま
    残ること」なので、そこを作らせなければよい。この時点ではゲート本体の `npm ci` が
    済んでいて道具が揃っており、到達性を**その時点の事実として**見られる。
    止めてもゲートと記録は完了しており、記録は手元に残って次回の実行で載る。

    🔴 **push 権限の申告（`permissions.push`）は見ない。** 一度それで
    「確実に publish できない」を判定していたが、**撤回した**（実測 2 つ）:
    `permissions.push` は `contents:write` の申告で PR 作成に要る `pull_requests:write`
    とは別物であり、この環境では proxy が無認証でも `push:true` を返すため
    **止める側の枝に到達しなかった**。申告を解釈せず、到達したという事実だけを使う。

    ⚠️ 0 は publish の**保証ではない**。保護ブランチ・レビュー必須・App のスコープは
    ここに現れず、proxy が何でも 200 にする環境ではなおさら。確かなのは
    「到達できなかった」側だけで、それがこの検査の値打ちである。
