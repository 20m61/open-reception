...
**`eslint . --no-inline-config` との差分**を取り、「その抑止が無ければ報告されていた問題」を
そのまま列挙する。`react-hooks/exhaustive-deps` を隠している抑止が既知の 3 ファイル以外に
あれば FAIL する。ゲートの `--pr` / `--full` で走る（eslint を 2 回回すので `--fast` には載せない）。
...
**ESLint の文法を手写しすること自体が前提の誤り**だった。`--no-inline-config` の差分なら
ファイル・行・ruleId が ESLint の口から出るので、綴り・複数行・ルール名省略・区切り記号・
