...
🔴 **綴りを列挙しない。** 初版は固定文字列 1 綴り、次は 3 綴りを列挙したが、独立レビューは
そのたびに**別の綴り**で素通りさせた（行末形・ブロック形・ルール名なし・複数行ブロック）。
前提が 2 つ間違っていた —— **ルール名は省略でき**（省略すると全ルールが外れるので名前を
書くより強い）、**ディレクティブは 1 物理行に収まらない**（ブロックコメントは改行を跨ぐ）。

いまは**コメントを取り出して本文がディレクティブかを文法で判定する**ので、綴りの網羅に
依存しない。走査対象は `git ls-files src tests scripts eslint-rules *.ts *.mjs`
（旧版は拡張子フィルタが `.tsx?` のみで `.mjs` を、`src` のみで `scripts/**` を取りこぼしていた）。
