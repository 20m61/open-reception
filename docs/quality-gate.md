...
- 🔴 **読み取りコマンドの引数として現れただけではブロックしない (#960)。**
  ...
  1. **シェルの機能を一切使わない読み取りパイプラインなら、そこで通す。**
     `;` `&` `&&` `||` backtick `$(` `<(` `>(` `>`（`2>/dev/null` を除く）ヒアドキュメントを
     含まず、各段の先頭語が読み取り専用の道具（`git` は読み取りサブコマンドのみ）で、
     **検出語が位置引数として現れている**こと。option の値として渡す形
     （`git grep --open-files-in-pager=<検出語>`）は通さない ―― 読み取りの道具が
     それを**起動する**（独立レビューが実際に任意スクリプトを走らせて見せた）
  ...
  代償は誤ブロックが残ることだが、**穴を開けるより誤発火を残すほうが安い**。
  allowlist には他プロセスを起動しない道具しか入れない（`find` / `fd` の `-exec`、
  `sort --compress-program`、`awk` の `system()`、`less` / `more` の `!` と `LESSOPEN`、
  `xargs` はいずれも除外）。
  🔴 **判定に使う道具（`perl` / `jq`）が落ちたら deny 側へ倒す。** 空の判定結果を返すと
  ガードが丸ごと無言で無効化される（fail-open）。`jq` が落ちたときは payload を
  読めていないので、引用の除去そのものをやめて記号を空白へ潰して検査する
  （payload は JSON なのでコマンド全体が引用の中にあり、「引用＝散文」の規則が
  コマンドを丸ごと捨ててしまう）。
  あわせて、**空白を含まない引用は中身を残す**（変更前は
  `npx tsx "scripts/merge-pull-request.ts" 1` が素通りしていた ―― 迂回が「クォートを
  2 つ足す」で済んでいた）。脱出ハッチの代入は**塊の先頭**でのみ認める。

  回帰行列は `tests/hooks/pr-gate-guard.test.ts` の `EXECUTION_FORMS`（56 形）/
  `READ_FORMS`（35 形）で、**散文ではなくこの配列が正本**。同じ 91 形を 3 版へ当てた実測:

  | 版 | 実行形 56（ブロックすべき） | 読み取り形 35（通すべき） |
  | --- | --- | --- |
  | 変更前（`b9f9718`） | 54 / 56（引用形 2 が素通り） | 6 / 35 |
  | blacklist 版（`6bba78b`） | **44 / 56** | 31 / 35 |
  | whitelist 版（現在） | **56 / 56** | **35 / 35** |

  🔴 **blacklist 版は実行形について変更前より弱かった。** 「誤発火を消す」変更が抜け道を
  13 族ぶん開けており、**自分で当てた変異は全部 kill していた** ―― 族ごとの見落としは
  独立レビュー 2 周でしか出なかった。実装への変異 18 種は行列と静的検査で全 kill
  （allowlist の中身は振る舞いで殺せないので、コメントを外したうえで静的に縛る）。
