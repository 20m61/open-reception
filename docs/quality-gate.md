...
- 🔴 **読み取りコマンドの引数として現れただけではブロックしない (#960)。**
  ...
  `find ... -exec` も読み取りではない。
  同時に、**空白を含まない引用は引用符だけを外す**ようにした。変更前は
  `npx tsx "scripts/merge-pull-request.ts" 1` が素通りしており（実測）、迂回が
  「クォートを 2 つ足す」で済んでいた。空白を含む引用（＝散文・コミットメッセージ）は
  従来どおり落とす。脱出ハッチの代入は**塊の先頭**でのみ認める（`echo "VAR=1" && gh pr merge`
  を迂回にしない）。
  実行形の回帰行列は `tests/hooks/pr-gate-guard.test.ts` の `EXECUTION_FORMS`。
  **方式を替えたので変更前に止まっていた形を全部当て直した**（変更前 22/24 → 変更後 24/24。
  増えた 2 件は引用形の穴を塞いだぶん）。
