...
- 🔴 **読み取りコマンドの引数として現れただけではブロックしない (#960)。**
  ...
  **`.claude/rules/opus5-autonomous-loop.md`「値を調整している自分には気づけない」の実例。**
  ...
  回帰行列は `tests/hooks/pr-gate-guard.test.ts` の `EXECUTION_FORMS`（110 形）/
  `READ_FORMS`（41 形）/ `DELIBERATELY_BLOCKED`（7 形）/ `KNOWN_PRE_EXISTING_GAPS`（1 形）で、
  **散文ではなくこの配列が正本**。同じ 151 形を 7 版へ当てた実測:

  | 版 | 実行しうる形 110 | 読み取り形 41 |
  | --- | --- | --- |
  | 変更前（`b9f9718`） | 97 / 110 | 7 / 41 |
  | blacklist 版（`6bba78b`） | **62 / 110** | 38 / 41 |
  | whitelist 版（`1616e8f`） | **74 / 110** | 31 / 41 |
  | 走査版（`04ebbc1`） | **86 / 110** | 39 / 41 |
  | 走査版 2（`84dd9c0`） | **94 / 110** | 40 / 41 |
  | 非解釈版（`f78735f`） | **98 / 110** | 40 / 41 |
  | 現在 | **110 / 110** | **41 / 41** |

  🔴 **7 版目の教訓は「新しい機構を足さない」。** 6 版目はリダイレクト先を食う処理を足して
