...
**リネームは新旧**両方**を数える (#719)。** `--name-only` はリネームの新側しか返さないので、
...
`--name-status -z` に変えて旧側も採る。🔴 **2 つの形式で順序が逆**（`diff --name-status -z`
は `R100\0旧\0新\0` で旧が先、`status --porcelain -z` は `R  新\0旧\0` で新が先）。
