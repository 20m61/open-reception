...
- 🔴 **読み取りコマンドの引数として現れただけではブロックしない (#960)。**
  ...
  **`.claude/rules/opus5-autonomous-loop.md`「値を調整している自分には気づけない」の実例。**
  ...
  回帰行列は `tests/hooks/pr-gate-guard.test.ts` の `EXECUTION_FORMS`（95 形）/
  `READ_FORMS`（40 形）/ `DELIBERATELY_BLOCKED`（7 形）で、**散文ではなくこの配列が正本**。
  同じ 135 形を 6 版へ当てた実測:

  | 版 | 実行しうる形 95 | 読み取り形 40 |
  | --- | --- | --- |
  | 変更前（`b9f9718`） | 82 / 95 | 6 / 40 |
  | blacklist 版（`6bba78b`） | **55 / 95** | 37 / 40 |
  | whitelist 版（`1616e8f`） | **66 / 95** | 31 / 40 |
  | 走査版（`04ebbc1`） | **77 / 95** | 39 / 40 |
  | 走査版 2（`84dd9c0`） | **85 / 95** | 40 / 40 |
  | 非解釈版（現在） | **95 / 95** | **40 / 40** |
