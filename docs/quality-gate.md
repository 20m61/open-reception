...
「不変条件は片側しか主張しない。**下界**を併せて縛る」に当たる。いまは 3 つを主張する。**1・2 は `react-hooks/exhaustive-deps` に限ったスコープ**である
（repo 全体では抑止 13 件のうち 10 件が理由なしで、それは既存として許容している）:

1. **`exhaustive-deps` の抑止件数がファイルごとに期待と完全一致**（増えても減っても落ちる＝下界）
2. **`exhaustive-deps` の抑止に重なる suppression が、すべて理由を持つ**
3. **全ルールの抑止総数が期待と一致**（スコープは repo 全体）

行番号では縛らない（無関係な編集で行がずれるたびに偽の赤を出さないため）が、
**件数まで見るので「許可ファイルの中で 2 件目を足す」も止まる**。
...
`exhaustive-deps` も黙って消えるのに、lint・棚卸し・unit がすべて緑だった。
しかも**warning は減る**（74 → 65）ので `--max-warnings` は永久に発火しない。

🔴 **3 が要るのは、追跡ルールと重ならない無差別 disable が件数でしか見えないから。**
`suppressedMessages` は「どのルール名を書いたか」を返さないので、
`exhaustive-deps` に重ならない `/* eslint-disable */` は 2 を素通りする。総数を縛れば、
どのファイルの無差別 disable も必ずここを動かす（実測で 13 → 22 / 13 → 16）。
