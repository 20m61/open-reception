...
**例外は 1 つだけ: `CURSOR_API_KEY`**（2026-08-26 にユーザー判断で決定・設定済み）。
Cursor CLI をクラウドから使うために置いている。詳細と、原則を曲げた分を何が代わりに縛るかは
`docs/cursor-cli-worker.md` §1-B。ここでは 2 点だけ繰り返す:

- キーが在る＝**このリポジトリの全クラウドセッションが Cursor を撃てる**。境界を担うのは
  キーの不在ではなく、`docs/cursor-cli-worker.md` §2（並走させない）と §3（委譲の可否）である
- **STS と違って失効しない。** AWS の窓のように「期限切れが最後の砦」にはならないので、
  使わなくなったら**明示的に削除する**（消すか回すか以外に閉じ方が無い）

例外を増やすときは、ここと `docs/cursor-cli-worker.md` の両方を同時に直す。片方だけ直すと
**2 文書が矛盾したまま残り、どちらが正本か分からなくなる**（実際に 2026-08-26〜27 の間そうなっていた）。
