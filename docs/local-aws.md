...
🔴 **予約記号（✅ と 🔴 素通り）は、許した表の許した列にしか書けない。機械で縛ってある**（#1114）。
書けるのは**凡例表の `記号` 列**・**下表の `Moto` / `MiniStack` 列**・
`docs/development/local-aws-sandbox.md` の証拠表・`docs/adr/0010-...md` の対応表だけである。
matrix の `Notes` / `Real AWS 必須` 列も**対象外**なので、そこへ ✅ を書くと落ちる。
