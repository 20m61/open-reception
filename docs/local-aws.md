...
—— `docs/local-aws.md` / `docs/development/local-aws-sandbox.md` /
`docs/adr/0010-swappable-aws-emulator.md`。**行が 1 文字でも増減・変化すると落ちる**
（`RESERVED_MARK_INVENTORY`）。表の構造では判定していない ―― 引用・HTML・コードフェンス・
区切り行の書式で逃げられたため、**出現そのものを数え上げる**方式へ裏返した。

**縛っているのは「黙って増えない・黙って消えない」であって、主張の真偽ではない。**
真偽は下表と証拠表について `reconcileCapabilityDoc` が実測記録と突き合わせる。
それ以外の表（例: 下記「CDK はローカルで往復する」の `| 段 | 結果 |`）は `OK` を使う。
...
🔴 **⛔ と ? は予約されていない。** 凡例が定義する記号ではあるが、**正の対照だけで書ける判定**
なので（負の対照が無くても「通らなかった」とは言える）、どの表にも書ける。逆に言えば
**⛔ は「嘘はつかない」を意味しない** —— 呼び方を変えれば素通りする可能性は残る。

🔴 **範囲を縛れるのは記号だけである。** `Notes` 列へ**平文で**「負の対照つきで実測済み」と
書く嘘は検査を通る。区別を記号が担う設計（#1113 AC3）の裏返しなので、
**能力の主張は matrix と証拠表にしか書かない**こと。散文・箇条書きの記号は主張ではない。
