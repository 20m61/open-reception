...
4. 表示が「停止中（全端末で受付を停止）」に変わったことを確認する
   ...
   - **画面が結果を確認できないと言った場合**（「サーバーからの応答を読み取れませんでした」
     「サーバーに接続できませんでした」「上の表示は最新でない可能性があります」）は、
     **表示を確認手段として使わない**。手順 5 の実機目視へ直行して判定する。
     停止が確認できなければ押し直す（この操作は冪等で、二重に送っても害はない）。
     - ⚠️ **監査ログでは判定できない。** `/admin/audit` は操作の記録（`セキュリティ設定: 更新`）
       までで、`emergencyStop` の**値は画面にも CSV にも出ない** —— 画面の列は
       `src/components/admin/audit/AuditLogViewer.tsx` が 日時/操作/主体/対象 に固定し、
       CSV は `src/components/admin/audit/logic.ts` が 日時/操作/主体/対象種別/対象 ID に
       固定していて、どちらも `metadata` を出さない（値そのものは
       `GET /api/admin/audit` の応答には含まれるが、運用中に API を直接叩く前提を置かない）。
       同じ `security.updated` はフォーム保存でも記録されるので、行が増えたことは根拠にならない。
   - **画面がはっきり失敗と言った場合**（「緊急停止を有効にできませんでした。」）は、
     サーバが要求を**見たうえで**断っている（4xx）。**押し直しても同じ**なので、権限を
     確かめる（`viewer` ロールでは 403。手順 1 の権限要件）。権限が正しければ §2.2 の
     個別無効化ルート（`/admin/devices` / `/admin/kiosks`）へ切り替える。
   - **「要求が反映されませんでした。…」の場合**は、サーバが 200 を返しつつ**状態を変えて
     いない**と答えている。数秒おいて押し直し、改善しなければ手順 5 の実機目視で判定する。
   - **「サーバーがエラーを返しました。…」の場合**（5xx）は**断られたとは限らない**。
     停止が適用されたあとで監査ログの書き込みが落ちた、ゲートウェイが 502/504 を返した、
     といった経路がある。数秒おいて **1〜2 回押し直し**（冪等）、それでも同じなら手順 5 の
     実機目視で判定する。
   - 「送信中…」の間は往復中なので、押し直さずに待つ（10 秒の締切があり、必ず結果が出る）。
...
2. 表示が「通常稼働」に戻ったことを確認する。
   - **画面が結果を確認できないと言った場合**は §2.2 手順 4 と同じで、表示を確認手段に
     使わず手順 4 の実機目視で判定する。**解除側は「止まったままなのに戻ったと読む」＝
     受付が上がらない**ので、確認できなければ押し直す（冪等）。
   - **はっきり失敗と言われた場合**も §2.2 手順 4 と同じ分け方をする。4xx
     （「緊急停止を解除できませんでした。」）は押し直しても同じなので権限を確かめる。
     5xx（「サーバーがエラーを返しました。…」）は**断られたとは限らない**ので、数秒おいて
     1〜2 回押し直す。🔴 **解除側には §2.2 のような迂回路が無い**（個別無効化は止める方向の
     手段）。押し直しても上がらなければ、手順 4 の実機目視で状態を確かめたうえで
     エスカレーションする。
