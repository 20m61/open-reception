   ...
   - **画面が結果を確認できないと言った場合**（「サーバーからの応答を読み取れませんでした」
     ...
     **表示を確認手段として使わない**。手順 5 の実機目視へ直行して判定する。
     ...
     - ⚠️ **監査ログでは判定できない。** `/admin/audit` は操作の記録（`セキュリティ設定: 更新`）
       までで、`emergencyStop` の**値はどこにも出ない** —— 画面の列は
       ...
       固定していて、どちらも `metadata` を出さない。同じ `security.updated` は
       フォーム保存でも記録されるので、行が増えたことは根拠にならない。
   - **画面がはっきり失敗と言った場合**（「緊急停止を有効にできませんでした。」）は、
     サーバが要求を見たうえで断っている。**押し直しても同じ**なので、権限を確かめる
     （`viewer` ロールでは 403。手順 1 の権限要件）。権限が正しければ §2.2 の個別無効化
     ルート（`/admin/devices` / `/admin/kiosks`）へ切り替える。
