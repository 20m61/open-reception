...
4. 表示が「停止中（全端末で受付を停止）」に変わったことを確認する
   ...
   - **画面が結果を確認できないと言った場合**（「サーバーからの応答を読み取れませんでした」
     ...
     **表示を確認手段として使わない**。手順 5 の実機目視へ直行し、`/admin/audit` の
     `security.updated`（`emergencyStop: true`）で適用の有無を確かめる。停止が確定して
     いなければ押し直す（この操作は冪等で、二重に送っても害はない）。
   - 「送信中…」の間は往復中なので、押し直さずに待つ（10 秒で必ず結果が出る）。
