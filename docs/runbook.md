   ...
   - **画面が結果を確認できないと言った場合**（「サーバーからの応答を読み取れませんでした」
     ...
     **表示を確認手段として使わない**。手順 5 の実機目視へ直行して判定する。
     ...
     - ⚠️ **監査ログでは判定できない。** `/admin/audit` は操作の記録（`セキュリティ設定: 更新`）
       までで、`emergencyStop` の**値は画面に出ない**（`src/components/admin/audit/logic.ts`
       が列を 日時/操作/主体/対象 に固定しており、CSV にも `metadata` を出さない）。
       同じ `security.updated` はフォーム保存でも記録されるので、行の存在は根拠にならない。
