...
| **サーバ側の設定不備・障害** | **5xx `unavailable`** (#1123) | サーバー側の問題で登録できません。時間をおいても直らない場合は管理者へ連絡してください。（**再試行ボタンを出す**） |

> 署名検証は exp 切れと改ざんを区別できないため `invalid_token` に集約する（UI は「無効か期限切れ」と案内）。

> 🔴 **5xx を上の 4xx と同じ扱いにしない (#1123)。** `KIOSK_ENROLLMENT_SECRET` 未設定や
> バックエンド障害で 5xx になったとき「URL が無効です／**再発行してください**」と案内すると、
> 指示どおり再発行しても発行側が同じ鍵で落ちるので**設置者が袋小路をループする**。
> 判定は本文の `error` ではなく **status** で行う（CloudFront は 502/504 を HTML の hold page で
> 返すため、本文で判定すると 5xx が 4xx の扱いに落ちる）。述語は
> `src/domain/util/http-failure.ts` の `isServerSideFailure` に 1 本化してある。
