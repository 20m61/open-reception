...
**2 度目**の成功で、**dev が 25 日ぶりに `main` に追いついた**
（`git rev-list --count --since=2026-08-15 442aca3` = **197**）。

> 🔴 **引き継ぎメモの失敗一覧にあった「2026-09-05 ― 成功」は誤りである。** 同日の dev デプロイは
> `AccessDenied ... iam:PassRole` で**失敗**している（本書 354 / 488 / 658 行がその実測）。
> 同日の「実施記録: 2026-09-05」は**その失敗を受けた IAM 修正（ADR 決定 6 の撤回）の検証**であって、
> デプロイの成功記録ではない。**引き継いだ数字は、記録へ書く前にこの文書で裏を取ること。**
