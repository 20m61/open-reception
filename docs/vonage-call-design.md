...
- ✅ **応答種別を全部無効化したサイトで新文言が空の領域を指す面（#1137）は閉じた。**
  文言は `responsesShown`（＝いま画面にボタンが出ているか）を受け取って 1 文を落とし、
  空状態は「設定されていません」と明示する。e2e は seed ではなく GET `respond` の stub で
  0 件を作るので**機械で止まる**（`tests/e2e/staff-call-failure.spec.ts`）。
  🔴 **残る面**: 押した先が `409 action_disabled` で断られたときに「リンクの有効期限切れ」と
  出る（**#1127 の射程**。`staffFailureForStatus` が status だけを見る設計に触れる）
