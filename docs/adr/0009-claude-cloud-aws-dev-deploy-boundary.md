...
- ✅ **決定 6（PassRole のタグ条件）は 2026-09-05 に実際に AccessDenied を起こして撤回し、
  同日に代替案の実測まで済ませた。** 「初回デプロイで AccessDenied になる最有力候補」という
  予測は当たっていたが、**予測を確かめるはずの検査（runbook 4b 14〜16 / S21・S22）が
  空虚だった**ため、3 週間のあいだ「検証済み」と記録され続けた。渡し先 ARN スコープへ替え、
  下界の S23 / S24 を足した。**裏取りは 3 通り**: `S23` が両リージョンで `allowed`（タグ
  context 無し）／ 4b 14〜16 が期待どおり ／ **`continue-update-rollback` が
  `--resources-to-skip` なしで完了**（実 API 経路。ポリシーを直しただけで、前回
  `AccessDenied` で落ちた同じ操作が通った）。詳細は決定 6 の改訂ブロックと
  runbook「実施記録: 2026-09-05」。
